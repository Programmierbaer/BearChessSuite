...
{
    /// <summary>
    /// Interaktionslogik für ConfigureSentioSpectrumWindow.xaml
    /// </summary>
    public partial class ConfigureSentioSpectrumWindow : Window
    {
        ...
        private TabutronicSentioSpectrumLoader _loader;
        private string _lastButtonName = string.Empty;
        private int _currentIndex;
        private bool _loaded = false;

        public ConfigureSentioSpectrumWindow(Configuration configuration, bool useBluetooth, bool useBluetoothLE) : this(configuration, useBluetooth, useBluetoothLE, configuration.FolderPath) { }

        public ConfigureSentioSpectrumWindow(Configuration configuration, bool useBluetooth, bool useBluetoothLE, string configPath)
        {
            InitializeComponent();
            ...
            InitializeComponent();
            _fileName = Path.Combine(configPath, TabutronicSentioSpectrumLoader.EBoardName,
                                                $"{TabutronicSentioSpectrumLoader.EBoardName}Cfg.xml");
            ...
            try
            {
                _fileLogger = new FileLogger(Path.Combine(fileInfo.DirectoryName, "log", "TabutronicSentioSpectrumCfg.log"), 10, 10);
            ...
            if (_useBluetooth)
            {
                ...
                _portNames = SerialCommunicationTools
                             .GetBTComPort(TabutronicSentioSpectrumLoader.EBoardName, configuration, _fileLogger, useBluetooth, useBluetoothLE, false).ToList();
            ...
            if (_eChessBoardConfiguration.Debounce == 0)
            {
                _eChessBoardConfiguration.Debounce = 1;
            }
        ...
        private void ButtonShowDim_OnClick(object sender, RoutedEventArgs e)
        {
            if (_loader == null)
            {
                _loader = new TabutronicSentioSpectrumLoader();
