...
namespace www.SoLaNoSoft.com.BearChessWpfCustomControlLib
{
    ...
    public partial class SplashWindow : Window
    {
        ...
        public SplashWindow(SplashProgressControlContent[] allContents, bool showCancel) : this()
        {
            ...
            foreach (var controlContent in allContents.OrderBy(c => c.Identifier))
            {
                var progressControl = new SplashProgressControl(controlContent.ShowCancel);
        ...
        {
            foreach (var splashProgressControlContent in allContents)
            {
                if (_allControls.TryGetValue(splashProgressControlContent.Identifier, out var control))
                {
                    control.SetContent(splashProgressControlContent);
                }
            }
        }
        ...
        private void OnCancelButtonClick(object sender, RoutedEventArgs e)
        {
            ...
            foreach (var child in StackPanelProgressControl.Children)
            {
                var splashProgressControl = child as SplashProgressControl;
