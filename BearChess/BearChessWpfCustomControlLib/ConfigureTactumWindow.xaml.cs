...
namespace www.SoLaNoSoft.com.BearChessWpfCustomControlLib
{
    ...
    public partial class ConfigureTactumWindow : Window
    {
        ...
        public ConfigureTactumWindow(Configuration configuration, bool useBluetoothLE, string configPath)
        {
            ...
            {
                _eChessBoardConfiguration.Debounce = 3;
            }
            numericUpDownUserControlDelay.Value = _eChessBoardConfiguration.Debounce;
            _isInitialized = true;
        ...
        private void ButtonOk_OnClick(object sender, RoutedEventArgs e)
        {
            ...
            _eChessBoardConfiguration.Debounce = numericUpDownUserControlDelay.Value;
        ...
        private void SayCurrentHelpText()
        {
            ...
            _synthesizer?.SpeakAsync(_currentHelpText);
            _currentHelpText = $"{_rm.GetString("Delay")} {numericUpDownUserControlDelay.Value} {_rm.GetString("HalfSeconds")}";
            _synthesizer?.SpeakAsync(_currentHelpText);
        ...
        private void NumericUpDownUserControlDelay_OnGotFocus(object sender, RoutedEventArgs e)
        {
            if (_isInitialized)
            {
                _currentHelpText =
                    $"{_rm.GetString("Delay")} {numericUpDownUserControlDelay.Value} {_rm.GetString("HalfSeconds")}";
                _synthesizer?.SpeakAsync(_currentHelpText);
            }
        }

        private void NumericUpDownUserControlDelay_OnValueChanged(object sender, int e)
        {
            if (_isInitialized)
            {
                _synthesizer?.Clear();
                _currentHelpText = $"{numericUpDownUserControlDelay.Value} {_rm.GetString("HalfSeconds")}";
                _synthesizer?.SpeakAsync(_currentHelpText);
            }
        }
    }
