...
namespace www.SoLaNoSoft.com.BearChessWin
{
    ...
    public partial class BearChessMainWindow : Window
    {
        ...
        private class EngineScore
        {
            ...
            public bool LoseByScore
            {
                get;
                private set;
            }

            public bool LoseByMate
            {
                get;
                private set;
            }
        ...
        private class WaitForSelfMoving
        {
            public string FromFieldFieldName { get; set; }
            public string ToFieldFieldName { get; set; }
            public string Promote { get; set; }
            public string EngineName { get; set; }
            public string AddEngineName { get; set; }

            public override string ToString()
            {
                return $"{EngineName}: {FromFieldFieldName}{ToFieldFieldName}{Promote}";
            }
        }
        ...
        private Dictionary<string, EcoCode> _ecoCodes;
        ...
        private readonly ConcurrentQueue<IBookMoveBase> _openingTrainingMoves = new ConcurrentQueue<IBookMoveBase>();
        private readonly ConcurrentQueue<Move> _repertoireTrainingMoves = new ConcurrentQueue<Move>();
        ...
        private string _prevTrainingFenPosition;
        ...
        private RepertoireDatabase _currentDatabaseRepertoire;
        ...
        private string _ficsPath;
        private string _publishPath;
        private string _dbPath = string.Empty;
        private string _repertoireDBPath = string.Empty;
        private string _puzzleDBPath = string.Empty;
        ...
        private bool _loadLastOrDefaultBook;
        private bool _showBestBookMove;
        private bool _showBestMove;
        private bool _showBestMove2;
        private bool _showBestMove3;
        private bool _showBestMoveN;
        ...
        private bool _blindUserNoBoard = false;
        private string _currentHelpText;
        private bool _newGamePositionEvent;
        private bool _sdiLayout;
        private readonly CultureInfo _currentThreadCurrentUiCulture;
        ...
        private string _bcsPlayerWhite = string.Empty;
        private string _bcsPlayerBlack = string.Empty;
        ...
        private string _savedFenFileName = string.Empty;
        private EcoCode _trainingEcoCode = null;
        private BookInfo _trainingBookInfo = null;
        private OpeningBook _trainingOpeningBook = null;
        private string _trainingDatabaseName = null;
        private Thread _trainingThread = null;
        private bool _inTrainingMode = false;
        private bool _inRepertoireTrainingMode = false;
        private OpeningTrainingConfig _openingTrainingConfig;
        private RepertoireTrainingConfig _repertoireTrainingConfig;
        private static readonly Random LineRandomness = new Random();
        private string _blindQueryFieldNumber = string.Empty;
        private string _blindMoveFieldInput = string.Empty;
        private bool _clockUseExtern = false;
        private string _oldPublishSend = string.Empty;
        private string _oldBoardFendSend = string.Empty;
        private bool _tournamentMode = false;
        private Dictionary<string, string> _trainingFenCodes = new Dictionary<string, string>();
        private WaitForSelfMoving _waitForSelfMoving = null;
        private const double _initialChessBackgroundFieldSize = 57.8;
        private const double _initialChessFieldSize = 50.8;
        private const double _initialFeeSpace = 632.04;
        private string _lastMove = string.Empty;
        private PuzzleWindow _puzzleWindow;
        private PgnGame _puzzlePgnGame;
        private int _puzzlePgnGameMoveIndex = 0;
        private List<Move> _puzzleMoves = null;
        private bool _waitForPuzzlePosition = false;
        private int _puzzleHelpRequestedCount = 0;
        private DatabasePuzzle _currentDatabasePuzzle = null;
        private PuzzleSource _currentPuzzleSource = PuzzleSource.None;
        private NewPuzzleRequest  _currentNewPuzzleRequest = null;
        private string _prevFen1 = string.Empty;
        private string _prevFen2 = string.Empty;
        private Move _waitForPlayerConfirm = null;
        private DateTime _lastChessComDailyPuzzle = DateTime.MinValue;
        ...
        public BearChessMainWindow()
        {
            ...
            InitializeComponent();
            InitBearChess(new FileInfo(assembly.Location),assembly.GetName());
        
        }
        #region init

        private void InitBearChess(FileInfo fileInfo, AssemblyName assemblyName)
        {
            ...
            for (var i = 1; i < args.Length; i++)
            {
                ...
                if (args[i].Equals("-tournament", StringComparison.OrdinalIgnoreCase))
                {
                    _tournamentMode = true;
                    continue;
                }
                if (args[i].Equals("-default", StringComparison.OrdinalIgnoreCase))
                {
                    _tournamentMode = false;
                    _blindUserSaySelection = false;
                    _blindUserSayMoveTime = false;
                    _blindUserSayBestBookMove = false;
                    _configuration.SetBoolValue("blindUser", false);
                    _configuration.SetBoolValue("tournamentMode", false);
                    continue;
                }
            ...
            _blindUserNoBoard = _configuration.GetBoolValue("blindUserNoBoard", false);
            ...
            else
            {
                ...
                if (_configuration.StartPathNumber>0)
                {
                    Title += $" - {_configuration.StartPathNumber}";
                }
            }
            ...
            TabItemBooks.Visibility = _tournamentMode ? Visibility.Collapsed : Visibility.Visible;
            ...
            Width = _configuration.GetDoubleValue("MainWinWidth", "650");
            Height = _configuration.GetDoubleValue("MainWinHeight", "680");

            if (!_sdiLayout)
            {
                Top = _configuration.GetWinDoubleValue("MainWinTopMDI", Configuration.WinScreenInfo.Top,
                    SystemParameters.VirtualScreenHeight, SystemParameters.VirtualScreenWidth);
                Left = _configuration.GetWinDoubleValue("MainWinLeftMDI", Configuration.WinScreenInfo.Left,
                    SystemParameters.VirtualScreenHeight, SystemParameters.VirtualScreenWidth);
            ...
            Rect rect = new Rect(0, 0, SystemParameters.VirtualScreenWidth, SystemParameters.VirtualScreenHeight);
            if (!rect.Contains(new Point(Left,Top)) || (Top <= 0 && Left <= 0))
            {
            ...
            _publishPath = Path.Combine(_configuration.FolderPath, "publish");
             _dbPath = Path.Combine(_configuration.FolderPath, "db");
             _repertoireDBPath = Path.Combine(_dbPath, "rep");
             _puzzleDBPath = Path.Combine(_dbPath, "puzzle");
            ...
            if (!Directory.Exists(_repertoireDBPath))
            {
                Directory.CreateDirectory(_repertoireDBPath);
            }
            if (!Directory.Exists(_puzzleDBPath))
            {
                Directory.CreateDirectory(_puzzleDBPath);
                var oldPuzzleDB = Path.Combine(_dbPath, PuzzleDatabase.ImportDBFileNameOld);
                if (File.Exists(oldPuzzleDB))
                {
                    var newPuzzleDB = Path.Combine(_puzzleDBPath, PuzzleDatabase.ImportDBFileName);
                    try
                    {
                        File.Move(oldPuzzleDB, newPuzzleDB);
                    }
                    catch
                    {
                ...
                oldPuzzleDB = Path.Combine(_dbPath, PuzzleDatabase.BearChessDBFileNameOld);
                if (File.Exists(oldPuzzleDB))
                {
                    var newPuzzleDB = Path.Combine(_puzzleDBPath, PuzzleDatabase.BearChessDBFileName);
                    try
                    {
                        File.Move(oldPuzzleDB, newPuzzleDB);
                    }
            ...
            _playerElo = _configuration.GetConfigValue("playerElo", "700");
            ...
            if (_sdiLayout)
            {
                chessBoardUcGraphics.ControlButtonSize =
                    _configuration.GetDoubleValue("ChessBoardUcGraphicsControlButtonSizeSDI", "42.8");
            }
            else
            {
                chessBoardUcGraphics.ChessBackgroundFieldSize =
                    _configuration.GetDoubleValue("ChessBoardUcGraphicsChessBackgroundFieldSizeMDI", "57.8");
                chessBoardUcGraphics.ChessFieldSize =
                    _configuration.GetDoubleValue("ChessBoardUcGraphicsChessFieldSizeMDI", "50.8");
                chessBoardUcGraphics.ControlButtonSize =
                    _configuration.GetDoubleValue("ChessBoardUcGraphicsControlButtonSizeMDI", "42.8");
            }
            ...
            chessBoardUcGraphics.ShowSmallClocks(_configuration.GetBoolValue("showSmallChessClock", true));
            imageShowSmallChessClock.Visibility = _configuration.GetBoolValue("showSmallChessClock", true)
                ? Visibility.Visible
                : Visibility.Hidden;
            imageResizeChessboard.Visibility = _configuration.GetBoolValue("resizeChessboard", true) && _sdiLayout ? Visibility.Visible
                : Visibility.Hidden;
            menuItemResizeChessboard.IsEnabled =_sdiLayout;
            if (_installedFieldsSetup.ContainsKey(_currentBoardFieldsSetupId))
            {
                ...
                chessBoardUcGraphics.SetBoardMaterial(_currentBoardFieldsSetupId, boardFieldsSetup.WhiteFileName,
            ...
            _loadLastOrDefaultBook = _blindUser || _configuration.GetBoolValue("LoadLastOrDefaultBook", true);
            imageLoadDefaultBook.Visibility = _loadDefaultBook ? Visibility.Visible : Visibility.Hidden;
            imageLoadLastOrDefaultBook.Visibility = _loadLastOrDefaultBook ? Visibility.Visible : Visibility.Hidden;
            ReadInstalledEngines();

            ReadInstalledBooks();
            if (!_tournamentMode)
            {
                if (_loadLastOrDefaultBook)
                {
                    LoadLastOrDefaultBook();
                }
                else if (_loadDefaultBook)
                {
                    LoadDefaultBook();
                }
            }
...
//            var ecoCodes = ecoCodeReader.LoadArenaFile(@"c:\Temp\ecocodes7.txt", Thread.CurrentThread.CurrentUICulture);
            ...
            EcoCode[]  ecoCodes = _tournamentMode ? new EcoCode[0] : ecoCodeReader.Load(Thread.CurrentThread.CurrentUICulture);
            //ecoCodeReader.Save(ecoCodes, Thread.CurrentThread.CurrentUICulture);

            _ecoCodes = ecoCodes.ToDictionary(e => e.FenCode, e => e);
            _lastEBoard = _blindUser
                ? _configuration.GetConfigValue("LastEBoard", Constants.TabutronicTactum)
            ...
            _bcsTournament = string.Empty;
            _currentSite = string.Empty;
            _currentRound = string.Empty;
            _lastResult = "*";
            ...
            imageBluetoothClassicMChessLink.Visibility =
                _useBluetoothClassicChessLink ? Visibility.Visible : Visibility.Hidden;
            ...
            _useBluetoothLETabuTronicCernoSpectrum =
                _configuration.GetBoolValue("usebluetoothLETabuTronicCernoSpectrum", false);
            imageCernoSpectrumBluetoothLE.Visibility =
                _useBluetoothLETabuTronicCernoSpectrum ? Visibility.Visible : Visibility.Hidden;

            _useBluetoothLETabuTronicSentio = _configuration.GetBoolValue("usebluetoothLETabuTronicSentio", false);
            imageSentioBluetoothLE.Visibility =
                _useBluetoothLETabuTronicSentio ? Visibility.Visible : Visibility.Hidden;

            _useBluetoothLETabuTronicSentioSpectrum =
                _configuration.GetBoolValue("usebluetoothLETabuTronicSentioSpectrum", false);
            imageSentioSpectrumBluetoothLE.Visibility =
                _useBluetoothLETabuTronicSentioSpectrum ? Visibility.Visible : Visibility.Hidden;

            _useBluetoothLETabuTronicTactum = _configuration.GetBoolValue("usebluetoothLETabuTronicTactum", false);
            imageTactumBluetoothLE.Visibility =
                _useBluetoothLETabuTronicTactum ? Visibility.Visible : Visibility.Hidden;
            imageSentioBluetoothLETactum.Visibility =
                _useBluetoothLETabuTronicTactum ? Visibility.Visible : Visibility.Hidden;
            ...
            imageMChessLinkChesstimation.Visibility =
                _useChesstimationChessLink ? Visibility.Visible : Visibility.Hidden;
            ...
            _useBluetoothTabuTronicCernoSpectrum =
                _configuration.GetBoolValue("usebluetoothTabuTronicCernoSpectrum", false);
            imageCernoSpectrumBluetooth.Visibility =
                _useBluetoothTabuTronicCernoSpectrum ? Visibility.Visible : Visibility.Hidden;
            ...
            _useBluetoothTabuTronicSentioSpectrum =
                _configuration.GetBoolValue("useBluetoothTabuTronicSentioSpectrum", false);
            imageSentioSpectrumBluetooth.Visibility =
                _useBluetoothTabuTronicSentioSpectrum ? Visibility.Visible : Visibility.Hidden;
            ...
            _enableInternalEngine = !_tournamentMode && _configuration.GetBoolValue("enableInternalEngine", true);
            ...
            _showBestMoveOnAnalysisMode =
                bool.Parse(_configuration.GetConfigValue("showbestmoveonanalysismode", "true"));
            imageShowBestMoveOnAnalysisMode.Visibility =
                _showBestMoveOnAnalysisMode ? Visibility.Visible : Visibility.Hidden;

            _showNextMoveOnGameAnalysisMode =
                bool.Parse(_configuration.GetConfigValue("shownextmoveongameanalysismode", "true"));
            imageShowNextMoveOnGameAnalysisMode.Visibility =
                _showNextMoveOnGameAnalysisMode ? Visibility.Visible : Visibility.Hidden;

            _showBestMoveOnGame = !_blindUser && !_tournamentMode && _configuration.GetBoolValue("showbestmoveongame", false);
            imageShowBestMoveOnGame.Visibility = _showBestMoveOnGame ? Visibility.Visible : Visibility.Hidden;

            _showRequestForHelp = !_tournamentMode && _configuration.GetBoolValue("showRequestForHelp", true);
            imageShowRequestForHelp.Visibility = _showRequestForHelp ? Visibility.Visible : Visibility.Hidden;

            _loadBuddyEngineOnGameStart = !_tournamentMode && _configuration.GetBoolValue("loadBuddyEngineOnGameStart",false);
            ...
            _speechLanguageTag = _configuration
            ...
            _showBestMove = _configuration.GetBoolValue("ShowBestMove", false);
            _showBestMove2 = _configuration.GetBoolValue("ShowBestMove2", false);
            _showBestMove3 = _configuration.GetBoolValue("ShowBestMove3", false);
            _showBestMoveN = _configuration.GetBoolValue("ShowBestMoveN", false);
            _arrowColor = _configuration.GetColorValue("ArrowColor", Brushes.Khaki);
            _bookMoveArrowColor = _configuration.GetColorValue("BookArrowColor", Brushes.CadetBlue);
            ...
            if (_tournamentMode)
            {
                menuItemConnectFics.Visibility = Visibility.Collapsed;
                menuItemTournament.Visibility = Visibility.Collapsed;
                menuItemEngines.Visibility = Visibility.Collapsed;
                menuItemBooks.Visibility = Visibility.Collapsed;
                menuItemAnalyseMode.Visibility = Visibility.Collapsed;
                menuItemTraining.Visibility = Visibility.Collapsed;
                menuItemDuel.Visibility = Visibility.Collapsed;
                menuItemPosition.Visibility = Visibility.Collapsed;
            ...
            {
                EngineWindowUserControl.Visibility = Visibility.Visible;
        ...
        private void ChessBoardUcGraphics_RequestForHintEvent(object sender, int e)
        {
            ...
            {
                moves = _chessBoard.GenerateMoveList()
                    .Where(m => m.ToField == e && m.FigureColor == _chessBoard.CurrentColor).ToList();
            }
            else
            {
            ...
            {
                chessBoardUcGraphics.MarkHintFields(new int[] { move.FromField, move.ToField }, _arrowColor);
            }
        }
        ...
        private void ChessBoardUcGraphics_SwitchColorEvent(object sender, EventArgs e)
        {
            ...
            chessBoardUcGraphics.SetPlayer(_currentGame.PlayerWhite, _currentGame.PlayerBlack, _currentGame.WhiteConfig.IsPlayer && _currentGame.BlackConfig.IsPlayer);
        ...
        private void ChessBoardUcGraphics_TakeFullForwardEvent(object sender, EventArgs e)
        {
            ...
            if (_showLastMove)
            {
                ...
                if (move != null)
                {
                    chessBoardUcGraphics.MarkLastMoveFields(new[]
                    {
                        move.FromField,
                        move.ToField
                    });
        ...
        private void ChessBoardUcGraphics_TakeStepBackEvent(object sender, EventArgs e)
        {
            ...
            if (_speechIsActive)
            {
                var move2 = _playedMoveList[_currentMoveIndex - 1];
                if (move2 != null)
                {
                    _synthesizer?.SpeakForce(_rm.GetString("TakeBack"));
                    MoveExtentions.SpeakMove(move2, true);
                }
            }
        ...
        {
            if (_currentAction == CurrentAction.InPuzzleMode)
            {
                return;
            }
            if (_currentAction == CurrentAction.InTrainingSetupMode)
            {
                return;
            }
            _engineWindow?.Stop();
            ...
            {
                var basePositionQueryWindow = new BasePositionQueryWindow(_chessBoard.GetPlayedMoveList().Length > 0)
                    { Owner = this };
            ...
            if (_pausedEngine && !_pausedGame)
            {
                ...
                else
                {
                    if (_chessBoard.CurrentColor == Fields.COLOR_WHITE)
                    {
                        ...
                        ClockWindowAndBoardGo(Fields.COLOR_WHITE);
                    ...
                    else
                    {
                       ...
                       ClockWindowAndBoardGo(Fields.COLOR_BLACK);                       
                    }
                }
            }
            else
        ...
        {
            if (_currentAction == CurrentAction.InAnalyseMode || _currentAction == CurrentAction.InEasyPlayingMode ||
            ...
            switch (_timeControlBlack.TimeControlType)
            {
                ...
                case TimeControlEnum.Adapted:
                {
                   ...
                   ClockWindowAndBoardGo(Fields.COLOR_BLACK);                   
                    if (goWithMoves)
                    {
                        _engineWindow?.GoCommandWithMoves(Fields.COLOR_BLACK,
                            $"wtime {second} btime {second} movestogo 9");
                    }
                    else
                    {
                ...
                case TimeControlEnum.TimePerMoves:
                {
                    var wTime = _chessClocksWindowWhite.GetClockTime().TotalSeconds * 1000;
                    var bTime = _chessClocksWindowBlack.GetClockTime().TotalSeconds * 1000;
                    _chessClocksWindowBlack.SetInfo(
                        $"{_timeControlBlack.Value1} {_rm.GetString("MovesIn")} {_timeControlBlack.Value2} {_rm.GetString("MovesMinutes")}");
                    _chessClocksWindowBlack.SetTooltip(
                        $"{_timeControlBlack.Value1} {_rm.GetString("MovesIn")} {_timeControlBlack.Value2} {_rm.GetString("MovesMinutes")}");
                   ClockWindowAndBoardGo(Fields.COLOR_BLACK);                   
                    ...
                    if (cycle2)
                    {
                        ...
                        {
                            _chessClocksWindowBlack.CorrectTime(hourH,
                                _timeControlBlack.Value2 + _timeControlBlack.HumanValue - hourH * 60, 0);
                        }
                        else
                        {
                    ...
                    {
                        _engineWindow?.GoCommandWithMoves(Fields.COLOR_BLACK,
                            $"wtime {wTime} btime {bTime} movestogo {movesToGo}");
                    }
                    else
                    {
                        _engineWindow?.GoCommand(Fields.COLOR_BLACK,
                ...
                {
                    _chessClocksWindowBlack.SetTooltip(
                        $"{_rm.GetString("SearchDepth")} {_timeControlBlack.Value1} {_rm.GetString("Plies")}");
                    _chessClocksWindowBlack.SetInfo(
                        $"{_rm.GetString("SearchDepth")} {_timeControlBlack.Value1} {_rm.GetString("Plies")}");
                   ClockWindowAndBoardGo(Fields.COLOR_BLACK);                   
                    if (goWithMoves)
                ...
                {
                    _chessClocksWindowBlack.SetTooltip(
                        $"{_rm.GetString("Search")} {_timeControlBlack.Value1} {_rm.GetString("Nodes")}");
                    _chessClocksWindowBlack.SetInfo(
                        $"{_rm.GetString("Search")} {_timeControlBlack.Value1} {_rm.GetString("Nodes")}");
                   ClockWindowAndBoardGo(Fields.COLOR_BLACK);                   
                    if (goWithMoves)
                ...
                case TimeControlEnum.Movetime:
                {
                   ...
                   ClockWindowAndBoardGo(Fields.COLOR_BLACK);                   
                    ...
                    {
                        _engineWindow?.GoCommandWithMoves(Fields.COLOR_BLACK,
                            $"movetime {_timeControlWhite.Value1 * 1000}");
                    }
                    else
                    {
        ...
        private void GoTimeForWhiteEngine(bool goWithMoves = false)
        {
            ...
            switch (_timeControlWhite.TimeControlType)
            {
                ...
                case TimeControlEnum.Adapted:
                {
                    ...
                    if (goWithMoves)
                    {
                        _engineWindow?.GoCommandWithMoves(Fields.COLOR_WHITE,
                            $"wtime {second} btime {second} movestogo 9");
                    }
                    else
                    {
                ...
                {
                    _chessClocksWindowWhite.SetTooltip(
                        $"{_rm.GetString("SearchDepth")} {_timeControlWhite.Value1} {_rm.GetString("Plies")}");
                    _chessClocksWindowWhite.SetInfo(
                        $"{_rm.GetString("SearchDepth")} {_timeControlWhite.Value1} {_rm.GetString("Plies")}");
                    ClockWindowAndBoardGo(Fields.COLOR_WHITE);
                    if (goWithMoves)
                    {
                        _engineWindow?.GoCommandWithMoves(Fields.COLOR_WHITE,
                            $"{_rm.GetString("SearchDepth")} {_timeControlWhite.Value1}");
                    }
                    else
                    {
                        _engineWindow?.GoCommand(Fields.COLOR_WHITE,
                ...
                {
                    _chessClocksWindowWhite.SetTooltip(
                        $"{_rm.GetString("Search")} {_timeControlWhite.Value1} {_rm.GetString("Nodes")}");
                    _chessClocksWindowWhite.SetInfo(
                        $"{_rm.GetString("Search")} {_timeControlWhite.Value1} {_rm.GetString("Nodes")}");
                    ClockWindowAndBoardGo(Fields.COLOR_WHITE);                   
                    if (goWithMoves)
        ...
        private void ClockWindowAndBoardStop()
        {
            ...
            var wTime = _chessClocksWindowWhite.GetCurrentTime();
            var bTime = _chessClocksWindowBlack.GetCurrentTime();
            _eChessBoard?.SetClock(wTime.Hour, wTime.Minute, wTime.Second, bTime.Hour, bTime.Minute, bTime.Second, 0);
        }

        private void ClockWindowAndBoardGo(int color)
        {
            if (_chessClocksWindowWhite == null || _chessClocksWindowBlack == null)
            {
                return;
        ...
        {
            string puzzleToFieldName = Fields.GetFieldName(toField);
            if (_currentAction == CurrentAction.InPuzzleMode)
            {
                var puzzlePromoteFigureId = FigureId.NO_PIECE;
                var board = new ChessBoard();
                board.Init(_chessBoard);
                var fromFieldFigure = board.GetFigureOn(fromField);
                if (fromFieldFigure.FigureId == FigureId.WHITE_PAWN && puzzleToFieldName.EndsWith("8"))
                {
                    var promoteWindow = new PromoteWindow(Fields.COLOR_WHITE, _blindUser) { Owner = this };
                    promoteWindow.ShowDialog();
                    puzzlePromoteFigureId = promoteWindow.PromotionFigureId;
                }

                if (fromFieldFigure.FigureId == FigureId.BLACK_PAWN && puzzleToFieldName.EndsWith("1"))
                {
                ...
                board.MakeMove(fromField, toField, puzzlePromoteFigureId);
                HandlePuzzleFenPosition(board.GetFenPosition());
                return;
            }
            ...
            if (_currentMoveIndex < _playedMoveList.Length)
            {
                ...
                foreach (var move in _chessBoard.GetPlayedMoveList())
                {
                    ...
                    _engineWindow?.MoveMadeBy(move.FigureColor);
                ...
                if (!_pausedEngine)
                    Dispatcher?.Invoke(() =>
                    {
                        ...
                        if (sendToEngine)
                        {
                            _engineWindow?.SetFen(position, string.Empty);
                            _engineWindow?.GoInfinite();
                            _engineWindow?.MoveMadeBy(chessFigure.Color);
                        }
                    });
                _materialWindow?.ShowMaterial(_chessBoard.GetFigures(Fields.COLOR_WHITE),
                    _chessBoard.GetFigures(Fields.COLOR_BLACK), _chessBoard.GetPlayedMoveList());
            ...
            if (fromField.Equals(toField))
            {
                ...
                {
                    var moves = _chessBoard.GenerateMoveList().Where(f =>
                        f.FigureColor == _chessBoard.CurrentColor && f.ToField.Equals(fromField)).ToList();
                    if (moves.Count == 1)
                    {
                ...
                {
                    var moves = _chessBoard.GenerateMoveList().Where(f =>
                        f.FigureColor == _chessBoard.CurrentColor && f.FromField.Equals(fromField)).ToList();
                    if (moves.Count == 1)
                    {
                        fromField = moves[0].FromField;
                        toField = moves[0].ToField;
            ...
            if (!_chessBoard.MoveIsValid(fromField, toField))
            {
                if (!_chessBoard.MoveIsValid(toField, fromField))
                {
                    if (_speechIsActive)
                    {
                        _synthesizer.SpeakAsync($"{_rm.GetString("InvalidMove")}  {Fields.GetFieldName(fromField)} {Fields.GetFieldName(toField)}");
                    }
            ...
            if (_bearChessServerClient != null && _bearChessServerClient.IsSending && (_currentGame==null || !_currentGame.PublishGame ))
            {
                _bearChessServerClient?.SendToServer(new BearChessServerMessage()
                {
                    ActionCode = BCServerConstants.ActionFen, Message = _chessBoard.GetFenPosition(),
            ...
            if (_speechOwnMove)
            {
                ...
                MoveExtentions.SpeakMove(fromFieldFigureId, toFieldFigureId, fromFieldFieldName, toFieldFieldName,
                    promoteFigureId, prevMove?.GetMove(_chessBoard.EnemyColor).ShortMoveIdentifier, addInfo);
            }
            ...
            Dispatcher?.Invoke(() =>
            {
                chessBoardUcGraphics.ShowForceMove(true);
                _engineWindow?.CurrentColor(_chessBoard.CurrentColor);
                _engineWindow?.MoveMadeBy(_chessBoard.EnemyColor);
            });
            ...
            if (isInCheck.Equals("#"))
            {
                ...
                if (move1.FigureColor == Fields.COLOR_WHITE)
                {
                    if (_chessClocksWindowWhite != null)
                    {
                        ...
                        move1.ElapsedMoveTime =
                            $"{elapsedTime.Hour:D2}:{elapsedTime.Minute:D2}:{elapsedTime.Second:D2}";
                        var clockTime = _chessClocksWindowWhite.GetCurrentTime();
                        move1.ClockTime =
                            $"{clockTime.Hour:D2}:{clockTime.Minute:D2}:{clockTime.Second:D2}";
                    }
                }
                else
                {
                    if (_chessClocksWindowBlack != null)
                    {
                        ...
                        move1.ElapsedMoveTime =
                            $"{elapsedTime.Hour:D2}:{elapsedTime.Minute:D2}:{elapsedTime.Second:D2}";
                        var clockTime = _chessClocksWindowBlack.GetCurrentTime();
                        move1.ClockTime =
                            $"{clockTime.Hour:D2}:{clockTime.Minute:D2}:{clockTime.Second:D2}";
                    }
                }
                ...
                if (_lastResult.Equals("1-0"))
                {
                    _eChessBoard?.WhiteWins();
                }
                else
                {
                    _eChessBoard?.BlackWins();
                }
            ...
            if (_chessBoard.IsDraw)
            {
                ...
                if (_chessBoard.DrawByStalemate)
                {
                    drawReason = _rm.GetString("Stalemate");
                }
                ...
                else
                {
                    ...
                    if (move1.FigureColor == Fields.COLOR_WHITE)
                    {
                        if (_chessClocksWindowWhite != null)
                        {
                            ...
                            prevMove.GetMove(_chessBoard.EnemyColor).ClockTime = move1.ClockTime;
                        }
                    }
                    else
                    {
            ...
            if (_chessBoard.CurrentColor == Fields.COLOR_WHITE)
            {
                ...
                if (move1.FigureColor == Fields.COLOR_WHITE)
                {
                    if (_chessClocksWindowWhite != null)
                    {
                        ...
                        move1.ElapsedMoveTime =
                            $"{elapsedTime.Hour:D2}:{elapsedTime.Minute:D2}:{elapsedTime.Second:D2}";
                        var clockTime = _chessClocksWindowWhite.GetClockTime();
                        move1.ClockTime =
                            $"{clockTime.Hour:D2}:{clockTime.Minute:D2}:{clockTime.Second:D2}";
                        if (prevMove != null)
                        {
                ...
                else
                {
                    if (_chessClocksWindowBlack != null)
                    {
                        ...
                        move1.ElapsedMoveTime =
                            $"{elapsedTime.Hour:D2}:{elapsedTime.Minute:D2}:{elapsedTime.Second:D2}";
                        var clockTime = _chessClocksWindowBlack.GetClockTime();
                        move1.ClockTime =
                            $"{clockTime.Hour:D2}:{clockTime.Minute:D2}:{clockTime.Second:D2}";
                        if (prevMove != null)
                        {
        ...
        private void ShowClocks()
        {
            ...
            if (_chessClocksWindowWhite == null)
            {
                ...
                _chessClocksWindowWhite.SimpleClockMode = _clockUseExtern;
            }
            ...
            if (_chessClocksWindowBlack == null)
            {
                ...
                _chessClocksWindowBlack.SimpleClockMode = _clockUseExtern;
            }
        ...
        private void SetTimeControl()
        {
            ...
            HandlingClocks.SetClocks(_timeControlWhite, _timeControlBlack, _chessClocksWindowWhite,
                _chessClocksWindowBlack,
        ...
        private void ContinueAGame(bool runEngine = true)
        {
            ...
            if (isConnected.HasValue && isConnected.Value)
            {
                ...
                if (_eChessBoard.SelfControlled)
                {
                    if (_lastEBoard.Equals(Constants.ChessUp) || _lastEBoard.Equals(Constants.ChessUp2))
                    {
                        ...
                        {
                            _eChessBoard?.SetEngineColor(
                                whiteIsPlayer ? Fields.COLOR_BLACK : Fields.COLOR_WHITE);
                        }
                    }
                    ...
                    BearChessMessageBox.Show(_rm.GetString("PlaceAlOnCorrectPosition"),
                        _rm.GetString("ConfirmPosition"),
                        MessageBoxButton.OK,
                ...
                else
                {
                    ...
                    _fileLogger?.LogDebug($"Send to eBoard and wait for: {fenPosition}");
                    _eChessBoard?.SetFen(fenPosition, string.Empty);
                    var splashProgressControlContents =
                        ProgressWorker.GetInitialContent(1, true, _rm.GetString("WaitForBoardPosition"));
                    var progressWorker = new ProgressWorker(_rm.GetString("PlaceYourChessmen"),
                        splashProgressControlContents, false);
                    progressWorker.DoWorkWithModal(progress =>
                    {
                        var eboardFen2 = _eChessBoard?.GetBoardFen();
                        int invalidFieldsCount = 0;
                        while (!splashProgressControlContents[0].Cancel &&
                               !fenPosition.StartsWith(eboardFen2) ||
                               string.IsNullOrEmpty(eboardFen2))
                        {

                            invalidFieldsCount = SetLEDsDiff(fenPosition, eboardFen2, invalidFieldsCount);
                            _eChessBoard?.DisplayOnClock($" -{invalidFieldsCount} ");
                            eboardFen2 = _eChessBoard?.GetBoardFen();
                            Thread.Sleep(300);
                        }

                        _eChessBoard?.SetDemoMode(false);
                        _eChessBoard?.SetAllLEDsOff(true);
                        _eChessBoard?.DisplayOnClock($"Go");
                    }, null);
            ...
            menuItemTrainStartOpening.IsEnabled = false;
            ...
            chessBoardUcGraphics.AllowTakeBack();
            if (_currentGame.WhiteConfig.IsPlayer && _currentGame.BlackConfig.IsPlayer)
            ...
            if (_currentGame.ContinueGame)
            {
                ...
                foreach (var playedMove in _chessBoard.GetPlayedMoveList())
                {
                    ...
                    if (length == 0)
                    {
                        ...
                        {
                            _engineWindow?.MakeMove(playedMove.FromFieldName.ToLower(),
                                playedMove.ToFieldName.ToLower(),
                                FigureId.FigureIdToFenCharacter[playedMove.PromotedFigure], _currentGame.PlayerWhite);
                            _engineWindow?.AddMove(playedMove.FromFieldName.ToLower(), playedMove.ToFieldName.ToLower(),
                                FigureId.FigureIdToFenCharacter[playedMove.PromotedFigure], _currentGame.PlayerBlack);
                        }
                        else
                        {
                            _engineWindow?.MakeMove(playedMove.FromFieldName.ToLower(),
            ...
            {
                if (_currentGame.WhiteConfig.IsPlayer && !_currentGame.WhiteConfig.IsBCChessServer)
                {
                ...
                {
                    if (_currentGame.BlackConfig.IsPlayer && !_currentGame.BlackConfig.IsBCChessServer)
                    {
                        if (chessBoardUcGraphics.WhiteOnTop)
                        {
                            ChessBoardUcGraphics_RotateBoardEvent(this, null);
        ...
        private void StartANewGame(bool runEngine = true)
        {
            ...
            buttonConnect.IsEnabled = _currentAction != CurrentAction.InRunningGame;
            var whiteIsPlayer = _currentGame.WhiteConfig.IsPlayer;
            var whiteIsBCPlayer = _currentGame.WhiteConfig.IsBCChessServer;
            var blackIsPlayer = _currentGame.BlackConfig.IsPlayer;
            var blackIsBCPlayer = _currentGame.BlackConfig.IsBCChessServer;
            _currentWhitePlayer = _currentGame.PlayerWhite;
            _currentBlackPlayer = _currentGame.PlayerBlack;
            _timeControlWhite = _currentGame.TimeControl;
            _timeControlBlack = _currentGame.TimeControlBlack ?? _currentGame.TimeControl;
            if (!_currentGame.CasualGame)
            {
                _configuration.Save(_timeControlWhite, true, false);
                _configuration.Save(_timeControlBlack, false, false);
            }

            _currentEvent = _currentGame.GameEvent;
            _lastResult = "*";
            _engineMatchScore.Clear();
            ...
            chessBoardUcGraphics.ShowPauseGame(true);
            chessBoardUcGraphics.SetPlayer(_currentWhitePlayer, _currentBlackPlayer, _currentGame.WhiteConfig.IsPlayer && _currentGame.BlackConfig.IsPlayer);
            if (_speechIsActive)
            {
                _synthesizer.SpeakAsync(
                    $"{_currentWhitePlayer} {SpeechTranslator.GetAgainst(_speechLanguageTag, _configuration)} {_currentBlackPlayer}");
            }
            ...
            _eChessBoard?.AllowTakeBack(true);
            _pureEngineMatch = false;
            _pureEngineMatchStoppedByBearChess = false;
            _gameAgainstEngine = true;
            _playersColor[Fields.COLOR_WHITE] = false;
            ...
            if (whiteIsBCPlayer || blackIsBCPlayer)
            {
                _gameAgainstEngine = true;
            }
            if (_gameAgainstEngine)
            ...
            if (!blackIsPlayer || blackIsBCPlayer)
            {
                if (_engineWindow != null)
                {
                    if (_currentGame.BlackConfig.IsChessServer && !_currentGame.BlackConfig.IsBCChessServer)
                    {
                        ...
                        {
                            BearChessMessageBox.Show($"{_rm.GetString("CouldNotLoad")} {_currentGame.BlackConfig.Name}",
                                _rm.GetString("ErrorOnLoading"),
                                MessageBoxButton.OK, MessageBoxImage.Error);
                            StopGame();
                            return;
                        }
                    }
                    if (_currentGame.BlackConfig.IsBCChessServer)
                    {
                        if (!_engineWindow.LoadUciEngine(_currentGame.BlackConfig, _bearChessServerClient,
                                _chessBoard.GetPlayedMoveList(), true,
                                Fields.COLOR_BLACK, _currentGame.GameNumber))
                        {
                            BearChessMessageBox.Show($"{_rm.GetString("CouldNotLoad")} {_currentGame.BlackConfig.Name}",
                    ...
                    if (!_currentGame.DuelEngine && !_currentGame.BlackConfig.IsChessServer && !_currentGame.CasualGame)
                    {
                        _configuration.SetConfigValue("LastBlackEngine", _usedEngines[_currentBlackPlayer].Id);
                    }
                }
            ...
            {
                _playersColor[Fields.COLOR_BLACK] = true;
                if (!_currentGame.DuelEngine && !_currentGame.CasualGame)
                {
                    _configuration.SetConfigValue("LastBlackEngine", string.Empty);
                }
            }

            if (!whiteIsPlayer || whiteIsBCPlayer)
            {
                if (_engineWindow != null)
                {
                    if (_currentGame.WhiteConfig.IsChessServer && !_currentGame.WhiteConfig.IsBCChessServer)
                    {
                        if (!_engineWindow.LoadUciEngine(_currentGame.WhiteConfig, _ficsClient,
                                _chessBoard.GetPlayedMoveList(), true,
                                Fields.COLOR_WHITE, _currentGame.GameNumber))
                        {
                            BearChessMessageBox.Show($"{_rm.GetString("CouldNotLoad")} {_currentGame.WhiteConfig.Name}",
                                _rm.GetString("ErrorOnLoading"),
                                MessageBoxButton.OK, MessageBoxImage.Error);
                            StopGame();
                            return;
                    ...
                    if (_currentGame.WhiteConfig.IsBCChessServer)
                    {
                        if (!_engineWindow.LoadUciEngine(_currentGame.WhiteConfig, _bearChessServerClient,
                                _chessBoard.GetPlayedMoveList(), true,
                                Fields.COLOR_WHITE, _currentGame.GameNumber))
                        {
                            BearChessMessageBox.Show($"{_rm.GetString("CouldNotLoad")} {_currentGame.BlackConfig.Name}",
                    ...
                    if (!_currentGame.DuelEngine && !_currentGame.WhiteConfig.IsChessServer && !_currentGame.CasualGame)
                    {
                        _configuration.SetConfigValue("LastWhiteEngine", _usedEngines[_currentWhitePlayer].Id);
                    }
                }
            ...
            _chessClocksWindowWhite.CountDown = !_clockUseExtern;
            _chessClocksWindowBlack.CountDown = !_clockUseExtern;

            SetTimeControl();
            if (_blindUser && _blindUserSayMoveTime)
            {
                _synthesizer?.SpeakAsync(
                    $"{_rm.GetString("ClockWhite")}: {SpeechTranslator.GetClockTime(_chessClocksWindowWhite.GetClockTime())}");
                _synthesizer?.SpeakAsync(
                    $"{_rm.GetString("ClockBlack")}: {SpeechTranslator.GetClockTime(_chessClocksWindowBlack.GetClockTime())}");
            }
                ...
                _timeControlWhite.TimeControlType == TimeControlEnum.TimePerMoves)
            {
                if (_chessClocksWindowWhite != null)
                {
                    ...
                    _eChessBoard?.SetClock(clockTimeW.Hour, clockTimeW.Minute, clockTimeW.Second,
                        clockTimeB.Hour, clockTimeB.Minute, clockTimeB.Second, 0);
                }
            }
            else
            {
                _eChessBoard?.SetClock(0, 0, 0, 0, 0, 0, 0);
            ...
            if (runEngine && _gameAgainstEngine)
            {
                ...
                if (_playersColor[_chessBoard.CurrentColor])
                {
                    return;
                }
        ...
        private void MenuItemNewGame_OnClick(object sender, RoutedEventArgs e)
        {
            ...
            {
                if (_blindUser && !_blindUserNoBoard &&  (_eChessBoard == null || !_eChessBoard.IsConnected))
                {
                    _synthesizer?.SpeakAsync(_rm.GetString("NewGame"), true);
                    var dialog = new QueryDialogWindow(_rm.GetString("ConnectFirstQuery"));
                    dialog.Owner = this;
        ...
        private void StopGame()
        {
            ...
            _pureEngineMatch = false;
            chessBoardUcGraphics.UnMarkAllFields();
            chessBoardUcGraphics.SetPlayer(string.Empty, string.Empty, true);
            chessBoardUcGraphics.ShowSmallClocks(_configuration.GetBoolValue("showSmallChessClock", true));
            ...
            menuItemTrainStartOpening.IsEnabled = true;
            chessBoardUcGraphics.AllowTakeBack();
            ...
            if (_bearChessServerClient != null && _bearChessServerClient.IsSending)
            {
                _bearChessServerClient?.SendToServer(new BearChessServerMessage()
                {
                    ActionCode = BCServerConstants.ActionStopGame, Message = string.Empty, Tournament = _bcsTournament
                });
            }
        ...
        private bool NewGame()
        {
            ...
            _gameJustFinished = false;
            _pureEngineMatch = false;
            chessBoardUcGraphics.ShowSmallClocks(true);
            chessBoardUcGraphics.UnMarkAllFields();
            chessBoardUcGraphics.SetPlayer(whitePlayer: _bcsPlayerWhite, blackPlayer: _bcsPlayerBlack, isPlayerGame: true);
            _databaseGameFen.Clear();
            ...
            {
                newGameWindow = new NewGameWindow(_configuration, _bearChessServerConnected, _eChessBoard != null) { Owner = this };
            }
            ...
            if (_eChessBoard != null && _eChessBoard.Level?.Length > 1)
            {
                ...
                if (_usedEngines[Constants.Citrine].IsActive)
                {
                    ...
                    _usedEngines[Constants.Citrine]
                        .AddOptionValue("setoption name Level 1-8 value " + _eChessBoard.Level[3]);
                    _usedEngines[Constants.Citrine].AddOptionValue("setoption name Level Play value " +
                                                                   _eChessBoard.Level[0] + _eChessBoard.Level[1]);
            ...
            newGameWindow.SetNames(_usedEngines.Values.ToArray(),
                ...
                _configuration.GetConfigValue("LastBlackEngine", string.Empty), string.IsNullOrWhiteSpace(_bcsPlayerWhite) && string.IsNullOrWhiteSpace(_bcsPlayerBlack));
            if (_bearChessServerClient != null && _bearChessServerClient.IsSending)
            {
                if (string.IsNullOrWhiteSpace(_bcsPlayerWhite) && string.IsNullOrWhiteSpace(_bcsPlayerBlack) &&
                    string.IsNullOrWhiteSpace(_bcsTournament))
                {
                    _bcsTournament = $"BearChess {DateTime.Now.ToShortDateString()} {DateTime.Now.ToShortTimeString()}";
                }
            }
            newGameWindow.SetBCServerPlayer(_bcsPlayerWhite, _bcsPlayerBlack, _bcsTournament, _bcsOwnColor);
            ...
            newGameWindow.SetStartFromBasePosition(_startFromBasePosition ||
                                                   _chessBoard.IsBasePosition(_chessBoard.GetFenPosition()));
            if (_eChessBoard != null)
            {
            ...
            if (!newGameWindow.CasualGame)
            {
                _configuration.SetConfigValue("LastPlayerWhite", playerWhiteConfigValues.OriginName);
                _configuration.SetConfigValue("LastPlayerWhiteElo",
                    playerWhiteConfigValues.GetConfiguredElo().ToString());
                _configuration.SetConfigValue("LastPlayerBlack", playerBlackConfigValues.OriginName);
                _configuration.SetConfigValue("LastPlayerBlackElo",
            ...
            if (_bearChessServerClient != null && _bearChessServerClient.IsSending)
            {
                _bearChessServerClient?.SendToServer(new BearChessServerMessage()
                {
                    ActionCode = BCServerConstants.ActionNewGame, Message = string.Empty, Tournament = _bcsTournament
                });

                _bearChessServerClient?.SendToServer(new BearChessServerMessage()
                {
                    ActionCode = BCServerConstants.ActionPlayerWhite,
                    Message = playerWhiteConfigValues.IsPlayer
                        ? string.IsNullOrWhiteSpace(playerWhiteConfigValues.OriginName)
                            ? _playerName
                            : playerWhiteConfigValues.OriginName
                        : newGameWindow.PlayerWhite,
                    Tournament = _bcsTournament
                });
                _bearChessServerClient?.SendToServer(new BearChessServerMessage()
                {
                    ActionCode = BCServerConstants.ActionPlayerBlack,
                    Message =   playerBlackConfigValues.IsPlayer ? string.IsNullOrWhiteSpace(playerBlackConfigValues.OriginName) ? _playerName : playerBlackConfigValues.OriginName : newGameWindow.PlayerBlack,
                    Tournament = _bcsTournament
                });
            }
            ...
            _currentGame = new CurrentGame(playerWhiteConfigValues, playerBlackConfigValues, newGameWindow.GameEvent,
            ...
            _currentGame.CasualGame = newGameWindow.CasualGame;
            _currentGame.PublishGame = newGameWindow.PublishGame;
            _currentGame.PublishGameContinuously = newGameWindow.PublishGameContinuously;
            if (_currentGame.PublishGame)
            {
                if (_gamePublisher == null)
                {
                    ...
                    _gamePublisher = new GamePublisher(FtpClientFactory.GetFtpClient(_fileLogger), _publishPath,
                        string.Empty, path, fileName, _fileLogger);
                }
        ...
        private void MenuItemSettingsBoard_OnClick(object sender, RoutedEventArgs e)
        {
            ...
            _chessBoardSetupWindow = new ChessBoardSetupWindow(_configuration, _boardPath, _piecesPath,
                _installedFieldsSetup,
            ...
            else
            {
                ...
                var boardFieldsSetup = _installedFieldsSetup[_currentBoardFieldsSetupId];
                chessBoardUcGraphics.SetBoardMaterial(boardFieldsSetup.Id, boardFieldsSetup.WhiteFileName,
                    boardFieldsSetup.BlackFileName);
        ...
        {
            chessBoardUcGraphics.SetBoardMaterial(_chessBoardSetupWindow.BoardFieldsSetup.Id,
                _chessBoardSetupWindow.BoardFieldsSetup.WhiteFileName,
                _chessBoardSetupWindow.BoardFieldsSetup.BlackFileName);
            if (_chessBoardSetupWindow.BoardPiecesSetup.Id.Equals(Constants.BearChess,
        ...
        private void HandleAnalyseMode(object sender, RoutedEventArgs e, CurrentAnalyseMode analyseMode)
        {
            ...
            {
                BearChessMessageBox.Show(_rm.GetString("BoardNotSuitableAnalysis"), _rm.GetString("NotSuitable"),
                    MessageBoxButton.OK,
            ...
            _currentAction = _currentAction == CurrentAction.InAnalyseMode
                ? CurrentAction.InEasyPlayingMode
                : CurrentAction.InAnalyseMode;
            buttonConnect.IsEnabled = _currentAction != CurrentAction.InRunningGame;
            textBlockRunningMode.Text = _currentAction == CurrentAction.InAnalyseMode
                ? _rm.GetString("ModeAnalysing")
                : _rm.GetString("ModeEasyPlaying");
            ...
            if (_currentAction == CurrentAction.InAnalyseMode)
            {
                ...
                {
                    if (MessageBox.Show(_rm.GetString("AnalyseCurrentGame"), _rm.GetString("AnalyseAGame"),
                            ...
                            MessageBoxImage.Question) == MessageBoxResult.Yes)
                    {
                        ...
                        while (!isValidInput)
                        {
                            if (_eChessBoard != null && _eChessBoard.SelfMoving)
                            {
                                _eChessBoard.NewGame();
                            }
                    ...
                    if (_currentGame != null)
                    {
                        _moveListWindow?.SetPlayerAndResult(_currentGame, _gameStartFenPosition, _databaseGame!=null ? _databaseGame.PgnGame : null);
                    }
                }
                else
                {
            ...
            chessBoardUcGraphics.SetInAnalyzeMode(
                _currentAction == CurrentAction.InAnalyseMode || _currentAction == CurrentAction.InGameAnalyseMode,
                _chessBoard.GetFenPosition(), analyseMode == CurrentAnalyseMode.FreeAnalyseMode);
        ...
        private void MenuItemSetupPosition_OnClick(object sender, RoutedEventArgs e)
        {
            ...
            if (_currentAction == CurrentAction.InRunningGame || _currentAction == CurrentAction.InAnalyseMode ||
                _currentAction == CurrentAction.InGameAnalyseMode)
            {
                return;
            }
            ...
            var fenPosition = (_eChessBoard != null && _eChessBoard.PieceRecognition)
                ? _eChessBoard.GetBoardFen()
                : _chessBoard.GetFenPosition();
            ...
            {
                BearChessMessageBox.Show(_rm.GetString("PlaceAlOnCorrectPosition"), _rm.GetString("ConfirmPosition"),
                    MessageBoxButton.OK,
                    MessageBoxImage.Information);
                _eChessBoard.Ignore(false);
            }
        ...
        private void PositionSetupWindow_RotateBoardEvent(object sender, EventArgs e)
        {
            if (_eChessBoard != null)
            {
                ...
                Dispatcher?.Invoke(() =>
                {
                    textBlockWhiteKing.Visibility =
                        _eChessBoard.PlayingWithWhite ? Visibility.Visible : Visibility.Collapsed;
                    textBlockBlackKing.Visibility =
                        _eChessBoard.PlayingWithWhite ? Visibility.Collapsed : Visibility.Visible;
                });
            }
        }
        ...
        private void MenuItemSettingsMoves_OnClick(object sender, RoutedEventArgs e)
        {
            ...
            if (showDialog.HasValue && showDialog.Value)
            {
                ...
                _configuration.SetConfigValue("DisplayCountryType",
        ...
        {
            if (_currentAction == CurrentAction.InRunningGame || _currentAction==CurrentAction.InPuzzleMode
                || _inTrainingMode || _inRepertoireTrainingMode)
            {
                return;
            }
            ...
            if (_lastEBoard.Equals(Constants.TabutronicSentioSpectrum, StringComparison.OrdinalIgnoreCase))
            {
                MenuItemConnectSentioSpectrum_OnClick(sender, e);
                return;
            }
            ...
            if (_lastEBoard.Equals(Constants.ChessnutMove, StringComparison.OrdinalIgnoreCase))
            {
                MenuItemConnectChessnutMoveBoard_OnClick(sender, e);
            }
        ...
        private void MenuItemClose_OnClick(object sender, RoutedEventArgs e)
        {
            ...
            if (BearChessMessageBox.Show(_rm.GetString("CloseBearChess") + "?", _rm.GetString("CloseBearChess"),
                    MessageBoxButton.YesNo, MessageBoxImage.Question) == MessageBoxResult.Yes)
            {
        ...
        private void MenuItemGamesLoad_OnClick(object sender, RoutedEventArgs e)
        {
            if (_databaseWindow == null)
            {
                _databaseWindow = new DatabaseWindow(_configuration, _database, _chessBoard.GetFenPosition(),
                    _duelTournamentIsRunning, _fileLogger);
        ...
        private void MenuItemPuzzleGamesLoad_OnClick(object sender, RoutedEventArgs e)
        {
            
            {
                _puzzleDatabaseWindow = new PuzzleDatabaseWindow(_puzzleDBPath, _fileLogger);
                _puzzleDatabaseWindow.ShowDialog();
            }
        }
        ...
        private void MenuItemGamesSave_OnClick(object sender, RoutedEventArgs e)
        {
            ...
            var pgnCreator = new PgnCreator(_gameStartFenPosition, _configuration.GetDefaultPgnConfiguration());
            if (_moveListWindow != null)
            {
                foreach (var move in _moveListWindow.GetAllMoves())
                {
            ...
            var saveGameWindow = new SaveGameWindow(_playerName, _currentWhitePlayer, _currentBlackPlayer, _lastResult,
                    _currentEvent,
                    DateTime.Now.ToString("yyyy.MM.dd"), pgnCreator.GetMoveList())
                { Owner = this };
            ...
            if (showDialog.HasValue && showDialog.Value)
            {
                ...
                _currentGame.GameEvent = saveGameWindow.GameEvent;
                _currentGame.PlayerWhite = saveGameWindow.White;
                _currentGame.PlayerBlack = saveGameWindow.Black;
                if (_currentGame.WhiteConfig.IsPlayer)
                {
                    _currentGame.WhiteConfig.OriginName = saveGameWindow.White;
                }
                if (_currentGame.BlackConfig.IsPlayer)
                {
                    _currentGame.BlackConfig.OriginName = saveGameWindow.Black;
                }
                _lastResult = saveGameWindow.Result;
                ...
                {
                    _database.SaveTournamentGamePair(_currentTournamentId, gameId, _tournamentManager.GetPairing());
                }
        ...
        private void ShowMoveListWindow(PgnGame pgnGame)
        {
            if (_moveListWindow != null)
            {
                if (_currentGame != null)
                {
                    _moveListWindow?.SetPlayerAndResult(_currentGame, _gameStartFenPosition, pgnGame);
                }
        ...
        private void MenuItemGamesPaste_OnClick(object sender, RoutedEventArgs e)
        {
            ...
            if (pgnGame != null)
            {
                ...
                uciInfoWhite.SetElo(pgnGame.WhiteElo);
                var uciInfoBlack = new UciInfo()
                {
                    IsPlayer = true,
                    Name = pgnGame.PlayerBlack
                };
                uciInfoBlack.SetElo(pgnGame.BlackElo);
                ...
                DatabaseWindow_SelectedGameChanged(this,
                    new DatabaseGame(pgnGame, _chessBoard.GetPlayedMoveList(), _currentGame));

                ShowMoveListWindow(pgnGame);
            }
        ...
        private void LoadProbingEngine()
        {
            ...
            if (_enableInternalEngine && _eChessBoard != null && _eChessBoard.IsConnected &&
        ...
        private void MenuItemEngineLoad_OnClick(object sender, RoutedEventArgs e)
        {
            ...
            var selectInstalledEngineWindow =
                new SelectInstalledEngineWindow(
                    _installedEngines.Values.Where(ie => !ie.IsInternalBearChessEngine).ToArray(),
                    _configuration.GetConfigValue("LastEngine", string.Empty), _uciPath, _fileLogger)
                {
                    Owner = this
                };
            ...
            {
                BearChessMessageBox.Show(_rm.GetString("EngineNotSuitable"), _rm.GetString("NotSuitable"),
                    MessageBoxButton.OK,
                    MessageBoxImage.Error);
                return;
            }
        ...
        private void LoadLastEngine(string lastEngineByName)
        {
            ...
            {
                var firstOrDefault = _installedEngines.Values.FirstOrDefault(u =>
                    u.Name.Equals(lastEngineByName, StringComparison.OrdinalIgnoreCase));
        ...
        private void LoadEngine(UciInfo uciInfo, bool lookForBookMoves, bool asBuddyEngine)
        {
            ...
            if (!uciInfo.IsInternalBearChessEngine && uciInfo.ValidForAnalysis && !uciInfo.IsBuddy &&
                !uciInfo.IsProbing)
                chessBoardUcGraphics.ShowRobot(true);
        ...
        private void ReadInstalledEngines()
        {
            try
            {
                ...
                var player1UciInfo = new UciInfo()
                {
                ...
                player1UciInfo.SetElo(_playerElo);
                var player2UciInfo = new UciInfo()
                {
                    Id = Guid.Empty.ToString("N"),
                    Name = Constants.Player2,
                    IsPlayer = true,
                    OriginName = _playerName,
                    Author = Constants.BearChess
                };
                player2UciInfo.SetElo(_playerElo);
                UciInfo bcServerPlayerUciInfo = new UciInfo()
                {
                    Id = Guid.Empty.ToString("N"),
                    Name = Constants.BCServerPlayer,
                    IsPlayer = true,
                    IsChessServer = true,
                    IsBCChessServer = true,
                    OriginName = Constants.BCServerPlayer,
                    LogoFileName = "BearChessServer.png",
                    Author = Constants.BearChess
                };
                ...
                osaUciInfo.AddOption(
                    "option name Level 1-8 type combo default 3 var 1 var 2 var 3 var 4 var 5 var 6 var 7 var 8");
                osaUciInfo.AddOption(
                    "option name Level A-H type combo default A var A var B var C var D var E var F var G var H");

                citrineUciInfo.AddOption(
                    "option name Level Play type combo default AT var AN var AT var BE var EA var IN var FD var SD var TR");
                citrineUciInfo.AddOption(
                    "option name Level 1-8 type combo default 3 var 1 var 2 var 3 var 4 var 5 var 6 var 7 var 8");

                _installedEngines.Add(player1UciInfo.Name, player1UciInfo);
                _installedEngines.Add(player2UciInfo.Name, player2UciInfo);
                if (_withBCServer)
                {
                    _installedEngines.Add(bcServerPlayerUciInfo.Name, bcServerPlayerUciInfo);
                    if (!_usedEngines.ContainsKey(bcServerPlayerUciInfo.Name))
                    {
                        _usedEngines.Add(bcServerPlayerUciInfo.Name, bcServerPlayerUciInfo);
                    }
                }
                ...
                if (!_usedEngines.ContainsKey(player1UciInfo.Name))
                {
                    _usedEngines.Add(player1UciInfo.Name, player1UciInfo);
                }

                if (!_usedEngines.ContainsKey(player2UciInfo.Name))
                {
        ...
        private void EngineWinsByBearChess(string engineName, bool byScore, bool byMate)
        {
            ...
            if (_speechIsActive)
            {
                ...
                {
                    _synthesizer.Speak(
                        $"{engineName} {SpeechTranslator.GetWinsByMate(_speechLanguageTag, _configuration)}");
                }
                else
                {
                    _synthesizer.Speak(
                        $"{engineName} {SpeechTranslator.GetWinsByScore(_speechLanguageTag, _configuration)}");
                }
            }
            ...
            if (_blindUser)
            {
                ...
                _synthesizer?.SpeakAsync(byMate
                    ? $"{engineName} {_rm.GetString("WinsByMate")} "
                    : $"{engineName} {_rm.GetString("WinsByScore")}");
                _gameJustFinished = true;
            }
            else
            {
        ...
        private void Engine_MakeMoveEvent(int fromField, int toField, string promote, decimal score, string bestLine,
            string engineName, string bestLineBuddy, decimal scoreBuddy)
        {
            try
            {
                ...
                bool selfMoveingMode = _eChessBoard != null && _eChessBoard.SelfMoving && _eChessBoard.IsConnected;
                ...
                if (_chessBoard.MoveIsValid(fromField, toField))
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(promote))
                    {
                        ...
                        if (_blindUser)
                        {
                            ...
                            var figureName = english
                                ? FigureId.FigureIdToEnName[promoteFigureId]
                                : FigureId.FigureIdToDeName[promoteFigureId];
                    ...
                    if (prevMove != null)
                    {
                        Dispatcher?.Invoke(() =>
                        {
                            ...
                            if (string.IsNullOrWhiteSpace(promote))
                            {
                                MoveExtentions.SpeakMove(fromFieldFigureId, toFieldFigureId, fromFieldFieldName,
                                    toFieldFieldName,
                            ...
                            else
                            {
                                MoveExtentions.SpeakMove(fromFieldFigureId, toFieldFigureId, fromFieldFieldName,
                                    ...
                                    prevMove.GetMove(_chessBoard.EnemyColor).ShortMoveIdentifier, addInfo, false);
                            }
                        });
                ...
                if (isInCheck.Equals("#"))
                {
                    ...
                    if (!_currentGame.DuelEngine || _currentGame.DuelEnginePlayer)
                    {
                        ...
                        else
                        {
                            ...
                            Dispatcher?.Invoke(() =>
                            {
                                BearChessMessageBox.Show($"{_rm.GetString("Mate")} {_lastResult} ",
                    ...
                    else
                    {
                        ...
                        if (!_currentGame.DuelEngine || _currentGame.DuelEnginePlayer)
                        {
                            ...
                            else
                            {
                                Dispatcher?.Invoke(() =>
                                {
                                    BearChessMessageBox.Show($"{draw} ", _rm.GetString("GameFinished"),
                                        MessageBoxButton.OK,
                                        MessageBoxImage.Stop);
                                });
                ...
                Dispatcher?.Invoke(() =>
                {
                    ...
                    if (_pureEngineMatch)
                    {
                        ...
                        _waitForSelfMoving = null;
                        if (selfMoveingMode)
                        {
                            if (engineName.Equals(_currentGame.PlayerBlack))
                            {
                                _waitForSelfMoving = new WaitForSelfMoving()
                                {
                                    FromFieldFieldName = fromFieldFieldName,
                                    ToFieldFieldName = toFieldFieldName,
                                    Promote = promote,
                                    EngineName = _currentGame.PlayerWhite,
                                    AddEngineName = _currentGame.PlayerBlack
                                };
                            }
                            else
                            {
                                _waitForSelfMoving = new WaitForSelfMoving()
                                {
                                    ...
                                    ToFieldFieldName = toFieldFieldName,
                                    Promote = promote,
                                    EngineName = _currentGame.PlayerBlack,
                                    AddEngineName = _currentGame.PlayerWhite
                                };
                            }
                        }
                        else
                    ...
                    if (!_pureEngineMatch || (selfMoveingMode))
                    {
                        _eChessBoard?.SetAllLEDsOff(false);
                        _eChessBoard?.ShowMove(new SetLEDsParameter()
                        {
                            ...
                            DisplayString =
                                _chessBoard.GetPrevMove()?.GetMoveString(_eBoardLongMoveFormat, _displayCountryType)
                        }, _alternateFenMoveList.Select(f => f.Fen).ToArray());
                    }
                    ...
                    if (_chessBoard.CurrentColor == Fields.COLOR_WHITE)
                    {
                        ...
                        if (_currentAction == CurrentAction.InRunningGame && _pureEngineMatch && !selfMoveingMode)
                        {
                            GoTimeForWhiteEngine();
                        }
                    }
        ...
        {
            if (_currentGame == null || _gamePublisher == null)
            {
                return;
            }
            if (!_currentGame.PublishGame)
            ...
            var playedMoveList = _moveListWindow != null ? _moveListWindow.GetAllMoves() : _chessBoard.GetPlayedMoveList();
            ...
            var pgnGame = new PgnGame
            {
                ...
                PlayerWhite = _currentGame.WhiteConfig.IsPlayer ? string.IsNullOrWhiteSpace(_currentGame.WhiteConfig.OriginName) ? _playerName : _currentGame.WhiteConfig.OriginName : _currentGame.PlayerWhite,
                PlayerBlack = _currentGame.BlackConfig.IsPlayer ? string.IsNullOrWhiteSpace(_currentGame.BlackConfig.OriginName) ? _playerName : _currentGame.BlackConfig.OriginName : _currentGame.PlayerBlack,
                Result = _lastResult,
                GameDate = DateTime.Now.ToString("yyyy.MM.dd"),
                Round = _currentGame.Round.ToString(),
                WhiteElo = _currentGame.WhiteConfig.GetConfiguredElo().ToString(),
                BlackElo = _currentGame.BlackConfig.GetConfiguredElo().ToString(),
            };
            if (!string.IsNullOrWhiteSpace(_gameStartFenPosition))
            {
                pgnGame.AddValue("FEN", _gameStartFenPosition);
            ...
            var newSend = pgnGame.GetGame();
            if (newSend.Equals(_oldPublishSend))
            {
                return;
            }

            if (_bearChessServerClient != null && _bearChessServerClient.IsSending &&
                _configuration.GetBoolValue("BCSforFTP", true))
            {
                _bearChessServerClient.SendToServer(new BearChessServerMessage()
                    { ActionCode = BCServerConstants.ActionPublish, Message = newSend, Tournament = _bcsTournament });
                _bearChessServerClient.SendToServer(new BearChessServerMessage()
                    { ActionCode = BCServerConstants.ActionPublishFen, Message = _chessBoard.GetFenPosition(), Tournament = _bcsTournament });
                _oldPublishSend = newSend;
                return;
            }

            _gamePublisher?.Publish(newSend);
            _oldPublishSend = newSend;
        }

        private void AutoSaveGame()
        {
            if ( _currentGame == null || _currentGame.GameSaved)
            {
                return;
            }
            ...
            pgnGame.AddValue("WhiteElo", _currentGame.WhiteConfig?.GetConfiguredElo().ToString().Replace("-1", ""));
            pgnGame.AddValue("BlackElo", _currentGame.BlackConfig?.GetConfiguredElo().ToString().Replace("-1", ""));
            _databaseGame = new DatabaseGame(pgnGame, playedMoveList, _currentGame);
            _database.Save(_databaseGame, false);
            if (_currentGame.PublishGame)
            {
                PublishGame();
            }

            if (_blindUser && _configuration.GetBoolValue("blindAutoSaveGames", true))
            {
                ...
                _synthesizer?.SpeakAsync(
                    $"{_rm.GetString("GameSavedInFolder")} {_rm.GetString("Documents")} {_rm.GetString("as")}", true);
        ...
        private void HandleEngineDuel()
        {
            ...
            else
            {
                ...
                if (_currentDuel.AdjustEloWhite || _currentDuel.AdjustEloBlack)
                {
                    ...
                    {
                        BearChessMessageBox.Show(
                            $"{_rm.GetString("EstimatedEloBetween")} {_currentDuel.CurrentMinElo} {_rm.GetString("And")} {_currentDuel.CurrentMaxElo}",
                            _rm.GetString("EstimatedElo"), MessageBoxButton.OK, MessageBoxImage.Information);
                    });
                }
        ...
        private void EngineWindow_EngineEvent(object sender, EngineEventArgs e)
        {
            ...
            if ((e.FirstEngine || _pureEngineMatch) && e.FromEngine.Contains(" pv "))
            {
                if (!e.FromEngine.Contains(" multipv ") || e.FromEngine.Contains(" multipv 1 "))
                {
                    ...
                    {
                        s = strings[i];
                        if (_showAnalysisBar && !_buddyEngineLoaded && !e.BuddyEngine &&
                            strings[i].Equals("score", StringComparison.OrdinalIgnoreCase))
                        {
                            ...
                            if (scoreType.Equals("cp", StringComparison.OrdinalIgnoreCase))
                            {
                                        ...
                                        out var score))
                                {
                                    ...
                                    Dispatcher?.Invoke(() =>
                                    {
                                        chessBoardUcGraphics.DrawAnalyses((double)analysesScore);
                                    });
                                }
                            }
                        }

                        if (!_buddyEngineLoaded && !e.BuddyEngine &&
                            strings[i].Equals("score", StringComparison.OrdinalIgnoreCase))
                        {
                            var scoreType = strings[i + 1];
                            if (scoreType.Equals("cp", StringComparison.OrdinalIgnoreCase))
                        ...
                        if (s.Equals("pv"))
                        {
                            if (_eChessBoard != null && !_buddyEngineLoaded && !e.BuddyEngine && _showRequestForHelp &&
                                _eChessBoard.Configuration.ShowHintMoves)
                            {
                                    ...
                                    if (strings[i + 2].Length == 4)
                                    {
                                        ...
                                        if ((_requestForHelpByEvent || _showHelp) && _requestForHelpArray.Length > 0)
                                        {
                                            ...
                                            if (eMove != null)
                                            {
                                                ...
                                                moveString =
                                                    $"* {allMoveClass.GetMoveString(_eBoardLongMoveFormat, _displayCountryType)}";
                                            }
                            ...
                            _bestLine = e.FromEngine.Substring(e.FromEngine.IndexOf(" pv ",
                            ...
                            try
                            {
                                if (strings[i + 1].Length >= 4)
                                {
                                    Dispatcher?.Invoke(() =>
                                    {
                                        if (_currentAction != CurrentAction.InGameAnalyseMode)
                                        {
                                            if (_showBestMove && e.FirstEngine)
                                            {
                                                //_fileLogger?.LogDebug($"Color Index 0 for first engine {e.Name}");
                                                chessBoardUcGraphics.MarkFirstEngineFields(new[]
                                                    {
            ...
            if (_showBestMove && _currentAction != CurrentAction.InGameAnalyseMode && !e.FirstEngine &&
                e.FromEngine.Contains(" pv ") &&
                (!e.FromEngine.Contains(" multipv ") || e.FromEngine.Contains(" multipv 1 ")))
            {
                var bestLine =
                    e.FromEngine.Substring(e.FromEngine.IndexOf(" pv ", StringComparison.OrdinalIgnoreCase) + 4);
                if (bestLine.Contains("  "))
                {
                    bestLine = bestLine.Substring(0, bestLine.IndexOf("  "));
                }

                Dispatcher?.Invoke(() =>
                {
                    if (!e.FirstEngine && bestLine.Length >= 4)
                    {
                        int index = e.EngineIndex;
                        bool showArrow = true;
                        switch (index)
                        {
                            case 0: showArrow = _showBestMove; break;
                            case 1: showArrow = _showBestMove2; break;
                            case 2: showArrow = _showBestMove3; break;
                            case 3: showArrow = _showBestMoveN; break;
                            default: showArrow = _showBestMoveN; break;
                        }

                        if (showArrow)
                        {
                            //_fileLogger?.LogDebug($"Color Index {index} for {buddyStr} engine {e.Name} ");
                            var color = GetColor(index, Brushes.Khaki);
                            chessBoardUcGraphics.RemoveArrows(color);
                            chessBoardUcGraphics.MarkFields(new[]
                                {
                                    Fields.GetFieldNumber(
                                        bestLine.Substring(0, 2)),
                                    Fields.GetFieldNumber(
                                        bestLine.Substring(2, 2))
                                },
                                color, index);
                        }
                    }
                ...
                && (!e.FromEngine.Contains(" multipv ") || e.FromEngine.Contains(" multipv 1 ")))
            {
                ...
                {
                    _bestLineBuddy = _bestLineBuddy.Substring(0, _bestLineBuddy.IndexOf("  "));
                }
            ...
            if (_eChessBoard != null && !e.FirstEngine && !_pureEngineMatch && e.FromEngine.Contains(" pv "))
            {
                if (!e.FromEngine.Contains(" multipv ") || e.FromEngine.Contains(" multipv 1 "))
                {
                    for (var i = 0; i < strings.Length; i++)
                    {
                        if (_buddyEngineLoaded && e.BuddyEngine && _showRequestForHelp &&
                            _eChessBoard.Configuration.ShowHintMoves && strings[i].Equals("pv"))
                        {
                            if (strings[i + 1].Length == 4)
                            {
                                _requestForHelpArray = new[]
                        ...
                        lock (_lockProbing)
                        {
                            if (!_eChessBoard.Configuration.ShowPossibleMoves &&
                                !_eChessBoard.Configuration.ShowPossibleMovesEval)
                            {
                                continue;
                            }
                            ...
                            if (_showProbing && _probingSend && (_probingDepth == _propingDepthTarget + 5) &&
                                strings[i].Equals("score", StringComparison.OrdinalIgnoreCase))
                            {
                            ...
                            {
                                if (_canSend && _probingEngineLoaded &&
                                    _currentProbingMoveListIndex < _probingMoveList.Length && e.ProbingEngine &&
                                    strings[i].Equals("score", StringComparison.OrdinalIgnoreCase))
                                {
                                    var scoreType = strings[i + 1];
                                    if (scoreType.Equals("cp", StringComparison.OrdinalIgnoreCase))
            ...
            if ((e.FirstEngine || _pureEngineMatch) &&
                strings[0].StartsWith("bestmove", StringComparison.OrdinalIgnoreCase) && strings[1].Length >= 4)
            {
                Dispatcher?.Invoke(() =>
                {
                    chessBoardUcGraphics.UnMarkAllFields();
            ...
            if (_pureEngineMatch || e.FirstEngine)
            {
                if (strings[0].StartsWith("Score", StringComparison.OrdinalIgnoreCase) && strings[1].Length >= 1)
                {
                    ...
                    else if (decimal.TryParse(strings[1].Replace(".", ","), NumberStyles.Any,
                                 CultureInfo.CurrentCulture,
        ...
        private void ReadInstalledMaterial()
        {
            try
            {
                _currentBoardFieldsSetupId =
                    _configuration.GetConfigValue("CurrentBoardFieldsSetupId", Constants.BearChess);
                ...
                if (!_installedFieldsSetup.ContainsKey(_currentBoardFieldsSetupId))
                {
                    if (!_currentBoardFieldsSetupId.Equals(Constants.Certabo) &&
                        !_currentBoardFieldsSetupId.Equals(Constants.Tabutronic) &&
                        !_currentBoardFieldsSetupId.Equals(Constants.BearChessWood))
                    {
                        _currentBoardFieldsSetupId = Constants.BearChess;
                    }
                }
                ...
                _currentBoardPiecesSetupId =
                    _configuration.GetConfigValue("CurrentBoardPiecesSetupId", Constants.BearChess);
        ...
        private void LoadLastOrDefaultBook()
        {

            var configValue = _configuration.GetConfigValue("lastBook", Constants.InternalBookGUIDPerfectCTG);
        ...
        private void LoadBook(BookInfo bookInfo)
        {
            ...
            _configuration.SetConfigValue("lastBook", bookInfo.Id);
            _configuration.Save();
            ...
            Dispatcher?.Invoke(() =>
            {
                ...
                if (bookMoveBase != null)
                {
                    ...
                    chessBoardUcGraphics.MarkBookFields(
                        new int[]
                        {
                            Fields.GetFieldNumber(bookMoveBase.FromField), Fields.GetFieldNumber(bookMoveBase.ToField)
                        },
                        _bookMoveArrowColor);
                    _eChessBoard?.SetLEDsFor(new SetLEDsParameter()
                        { BookFieldNames = new[] { bookMoveBase.FromField, bookMoveBase.ToField } });
                }
                ...
                if (_blindUser && !string.IsNullOrEmpty(textBlockBestBookMove.Text))
                {
                    ...
                    MoveExtentions.SpeakMove(figureOnField.FigureId, figureOnToField.FigureId,
                        bookMoveBase.FromField.ToUpper(),
        ...
        {
            ConnectDisconnectToBoard(ConnectToSquareOffPro, DisconnectFromSquareOffPro);
        ...
        private void ConnectToChessUp()
        {
            ...
            if (!_eChessBoard.IsConnected)
            {
                ...
                BearChessMessageBox.Show(_rm.GetString("CheckConnection"), _rm.GetString("ConnectionFailed"),
                    MessageBoxButton.OK,
                    MessageBoxImage.Error);
                return;
            }
        ...
        {
            ConnectDisconnectToBoard(ConnectToPegasus, DisconnectFromPegasus);
        }
        ...
        private void DisconnectFromChessnutMove()
        {
            DisconnectFromEBoard(menuItemConnectToChessnutAirBoard, "Chessnut Move");
        }
        ...
        private void ConnectToChessnutMove()
        {
            _fileLogger?.LogInfo($"Connect to Chessnut Move chessboard");

            _eChessBoard = new ChessnutMoveLoader(_configuration.FolderPath);
            AddEBoardEvents();
            if (!_eChessBoard.IsConnected)
            {

                DisconnectFromChessnutMove();
            ...
            menuItemConnectToChessnutMoveBoard.Header = _rm.GetString("Disconnect");
            DisableConnectItems();
            menuItemChessnutMoveBoard.IsEnabled = true;
            _eChessBoard.SetDemoMode(_currentAction == CurrentAction.InAnalyseMode ||
                                     _currentAction == CurrentAction.InEasyPlayingMode ||
                                     _currentAction == CurrentAction.InGameAnalyseMode);
            var currentComPort = _eChessBoard?.GetCurrentComPort();
            ...
            imageBT.Visibility = currentComPort.Equals("BTLE", StringComparison.OrdinalIgnoreCase)
                ? Visibility.Visible
                : Visibility.Hidden;
            _lastEBoard = Constants.ChessnutMove;
            textBlockButtonConnect.Text = _eChessBoard.Information;
            buttonConnect.Visibility = Visibility.Visible;
            imageConnect.Visibility = Visibility.Collapsed;
            imageDisconnect.Visibility = Visibility.Visible;
        ...
        private void ConnectToDGT()
        {
            ...
            _eChessBoard.Calibrate();
            _eChessBoard.DisplayOnClock("* Bear *");
            if (_currentAction == CurrentAction.InRunningGame)
            {
                _eChessBoard.SetFen(_chessBoard.GetFenPosition(), string.Empty);
            }
        ...
        private void DisconnectFromEBoard(MenuItem menuItem, string boardName)
        {
            ...
            _eChessBoard.ClockEvent -= EChessBoard_ClockEvent;
            _eChessBoard.SetAllLEDsOff(true);
            Thread.Sleep(100);
            ...
            _eChessBoard = null;
            textBlockEBoard.Text = $" {_rm.GetString("Disconnected")}";
        ...
        private void DisconnectFromTabutronicSentioSpectrum()
        {
            DisconnectFromEBoard(menuItemConnectToSentioSpectrum, Constants.TabutronicSentioSpectrum);
        }
        ...
        private void ConnectToTabutronicSentioSpectrum()
        {
            _fileLogger?.LogInfo("Connect to TabuTronic Sentio Spectrum chessboard");
            _eChessBoard = new TabutronicSentioSpectrumLoader(_configuration.FolderPath);
            AddEBoardEvents();
            if (!_eChessBoard.IsConnected)
            {
                DisconnectFromTabutronicSentioSpectrum();
                BearChessMessageBox.Show(_rm.GetString("CheckConnection"), _rm.GetString("ConnectionFailed"), MessageBoxButton.OK,
                    MessageBoxImage.Error);
                return;
            }
            ...
            _eChessBoard.SetDemoMode(_currentAction == CurrentAction.InAnalyseMode ||
                                     ...
                                     _currentAction == CurrentAction.InGameAnalyseMode);
            var currentComPort = _eChessBoard?.GetCurrentComPort();
            textBlockEBoard.Text = $"{_rm.GetString("ConnectedTo")} {Constants.TabutronicSentioSpectrum} ({currentComPort})";
            imageBT.Visibility = currentComPort.Equals("BT", StringComparison.OrdinalIgnoreCase)
                                     ? Visibility.Visible
                                     : Visibility.Hidden;
            _lastEBoard = Constants.TabutronicSentioSpectrum;
            textBlockButtonConnect.Text = _lastEBoard;
            buttonConnect.Visibility = Visibility.Visible;
            imageConnect.Visibility = Visibility.Collapsed;
            imageDisconnect.Visibility = Visibility.Visible;
            SetLastEBoard(_lastEBoard);
            buttonConnect.ToolTip = $"{_rm.GetString("DisconnectFrom")} {Constants.TabutronicSentioSpectrum}";
            if (_currentAction == CurrentAction.InRunningGame)
            {
                _eChessBoard.SetFen(_chessBoard.GetFenPosition(), string.Empty);
            }
        ...
        private void ConnectToTabutronicCerno()
        {
            ...
            _eChessBoard.SetDemoMode(_currentAction == CurrentAction.InAnalyseMode ||
                                     ...
                                     _currentAction == CurrentAction.InGameAnalyseMode);
            var currentComPort = _eChessBoard?.GetCurrentComPort();
            textBlockEBoard.Text = $"{_rm.GetString("ConnectedTo")} {Constants.TabutronicCerno} ({currentComPort})";
        ...
        private void ConnectToTabutronicCernoSpectrum()
        {
            ...
            _eChessBoard.SetDemoMode(_currentAction == CurrentAction.InAnalyseMode ||
                                     _currentAction == CurrentAction.InEasyPlayingMode ||
                                    // _currentAction == CurrentAction.InTrainingMode ||
                                     _currentAction == CurrentAction.InGameAnalyseMode);
            var currentComPort = _eChessBoard?.GetCurrentComPort();
            textBlockEBoard.Text = $"{_rm.GetString("ConnectedTo")} {Constants.TabutronicCernoSpectrum} ({currentComPort})";
        ...
        private void EChessBoard_GameEndEvent(object sender, string e)
        {
            if (_pureEngineMatch || _waitForPosition || _currentAction == CurrentAction.InGameAnalyseMode)
            {
                return;
            ...
            if (_chessBoard.FullMoveNumber > 1)
            {
                Dispatcher?.Invoke(() =>
                {
                    ...
                    _eChessBoard?.Stop();
                    _eChessBoard?.SetAllLEDsOff(false);
                    if (_lastResult.Equals("1/2"))
                    {
                        BearChessMessageBox.Show($"{_rm.GetString("GameFinished")}  {_lastResult} ",
                            $"{_rm.GetString("DrawByPlayer")}",
                            MessageBoxButton.OK,
                            MessageBoxImage.Stop);
                    }
                    else if (_lastResult.StartsWith("1"))
                    {

                        BearChessMessageBox.Show($"{_rm.GetString("GameFinished")}  {_lastResult}",
                            $"{_rm.GetString("WhiteWon")}",
                            MessageBoxButton.OK,
                            MessageBoxImage.Stop);
                    }
                    else
        ...
        {
            if (_pureEngineMatch || _waitForPosition || _currentAction == CurrentAction.InGameAnalyseMode
                || _currentAction == CurrentAction.InTrainingSetupMode || _inTrainingMode)
            {
                return;
            }
        ...
        {
            ConnectDisconnectToBoard(ConnectToCertabo, DisconnectFromCertabo);
        }

        private void EChessBoardAlternateFenEvent(object sender, string fenPosition)
        {
            if (!_eChessBoard.PieceRecognition)
            {
                return;
            }
            if (_pureEngineMatch || _currentAction == CurrentAction.InTrainingSetupMode || _currentAction == CurrentAction.InPuzzleMode)
            {
                return;
            }
            ...
            _eChessBoard?.SetAllLEDsOff(true);
            _fileLogger?.LogDebug($"Alternate fen position from e-chessboard: {fenPosition}");
            if ( _currentAction != CurrentAction.InGameAnalyseMode && _currentAction != CurrentAction.InSetupMode)
            {
                var moveList = _chessBoard.GetPlayedMoveList();
                _chessBoard.TakeBack();
                _chessBoard.TakeBack();
                moveList = _chessBoard.GetPlayedMoveList();
                _eChessBoard?.SetFen(_chessBoard.GetFenPosition());
                var aMove = _chessBoard.GetMove(fenPosition, true);
                _fileLogger?.LogDebug($"Calcuated move from alternate fen position: {aMove}");
                if (!string.IsNullOrEmpty(aMove))
                {
                    Dispatcher?.Invoke(() =>
                    {
                        _engineWindow?.Stop();
        ...
        {            
            if (_pureEngineMatch || _currentAction==CurrentAction.InTrainingSetupMode || _waitForPosition)
            {
                return;
            }
            ...
            if (fenPosition.Contains("/"))
            {
                ...
                if (fenPosition.Contains("k") && fenPosition.Contains("K"))
                {
                    ...
                    else
                    {
                        Dispatcher?.Invoke(() =>
                        {
                            if (GridEBoardControl.Visibility == Visibility.Visible)
                            {
                                _eBoardControlWindow?.SetFen(fenPosition);
                            }
                        });
            ...
            if (_currentAction == CurrentAction.InSetupMode || 
                _currentAction == CurrentAction.InAnalyseMode || 
                _currentAction == CurrentAction.InEasyPlayingMode ||
               _currentAction == CurrentAction.InPuzzleMode ||
                _currentAction == CurrentAction.InGameAnalyseMode)
            {
                if (!_waitForPosition)
                {
            ...
            if (_currentGame != null && _currentGame.PublishGame && _bearChessServerClient != null && _bearChessServerClient.IsSending
                && _configuration.GetBoolValue("BCSforFTP", true))
            {
                if (!_oldBoardFendSend.Equals(fenPosition))
                {
                    _fileLogger?.LogDebug($"Send BOARDFEN: {fenPosition}");
                    _bearChessServerClient.SendToServer(new BearChessServerMessage()
                        { ActionCode = BCServerConstants.ActionBoardFen, Message = fenPosition, Tournament = _bcsTournament });
                    _oldBoardFendSend = fenPosition;
                }
            }
        }
        ...
        private void EChessBoardAwaitedPositionEvent(object sender, EventArgs e)
        {
            ...
            if (_pureEngineMatch)
            {
                if (_eChessBoard != null && _eChessBoard.SelfMoving && _eChessBoard.IsConnected && _waitForSelfMoving!=null)
                {
                    Dispatcher?.Invoke(() =>
                    {
                        _fileLogger?.LogDebug(
                            $"Awaited position from self moving chessboard. Make move {_waitForSelfMoving}");
                        _engineWindow?.MakeMove(_waitForSelfMoving.FromFieldFieldName,
                            _waitForSelfMoving.ToFieldFieldName,
                            _waitForSelfMoving.Promote, _waitForSelfMoving.EngineName);
                        _engineWindow?.AddMove(_waitForSelfMoving.FromFieldFieldName,
                            _waitForSelfMoving.ToFieldFieldName,
                            _waitForSelfMoving.Promote, _waitForSelfMoving.AddEngineName);
                        _waitForSelfMoving = null;
                        _engineWindow?.CurrentColor(_chessBoard.CurrentColor);
                        if (_chessBoard.CurrentColor == Fields.COLOR_WHITE)
                        {
            ...
            if (_timeControlWhite != null && _currentAction == CurrentAction.InRunningGame)
            {
                ...
                Dispatcher?.Invoke(() =>
                {
                    ...
                    if (_timeControlBlack.WaitForMoveOnBoard)
                    {
                        GoTimeForBlackEngine();
                    }
                    return;
        ...
        {
            ConnectDisconnectToBoard(ConnectToChessLink, DisconnectFromChessLink);
        }

        private void MenuItemConfigureCertabo_OnClick(object sender, RoutedEventArgs e)
        {
            ConfigureEBoard(Constants.Certabo,ConnectToCertabo,DisconnectFromCertabo,new ConfigureCertaboWindow(_configuration, _useBluetoothCertabo, _useBluetoothLECertabo, _useChesstimationCertabo) {Owner = this});
        }
        ...
        {
            ConfigureEBoard(Constants.MChessLink,ConnectToChessLink,DisconnectFromChessLink,
                new ConfigureMChessLinkWindow(_configuration,_useBluetoothClassicChessLink, _useBluetoothLEChessLink, _useChesstimationChessLink, _useElfacunChessLink) {Owner = this});
        }
        ...
        private void EChessBoardFenEvent(string fenPosition)
        {
            ...
            if (_currentAction == CurrentAction.InPuzzleMode)
            {
                HandlePuzzleFenPosition(fenPosition);
                return;
            }
            ...
            var code = fenPosition.Split(" ".ToCharArray())[0];
            if (_inTrainingMode)
            {
                if (_trainingFenCodes.ContainsKey(code))
                {
                    fenPosition = _trainingFenCodes[code];
                }
            }
            ...
            if (_chessBoard.IsInCheck(_chessBoard.EnemyColor))
            {
                _fileLogger?.LogDebug($"Fen invalid because both colors are in check: {fenPosition}");
                _chessBoard.SetPosition(position, false);
                if (_eChessBoard.SelfMoving && _currentAction==CurrentAction.InRunningGame)
                {
                    _eChessBoard.SetFen(position);
                }
            }
            ...
            if (_inTrainingMode)
            {
                if (!_trainingFenCodes.ContainsKey(code))
                {
                    _trainingFenCodes.Add(code, fenPosition);
                }
            }
            Dispatcher?.Invoke(() =>
            {
                ...
                if (_inTrainingMode)
                {
                    if (_inRepertoireTrainingMode)
                    {
                        HandleRepertoireOpeningTraining(fenPosition, position);
                    }
                    else
                    {
                        HandleOpeningTraining(fenPosition, position);
                    }
                }
            });
        }
        ...
        private void EChessBoardMoveEvent(string move)
        {
            ...
            if (_pureEngineMatch || _currentAction==CurrentAction.InPuzzleMode)
            {
                return;
            }
            if (_requestForHelpByEvent && !_inTrainingMode)
            {
                Dispatcher?.Invoke(() =>
                {
                    _eChessBoard?.SetLEDsFor(new SetLEDsParameter()
            ...
            if (!_chessBoard.MoveIsValid(fromField, toField))
            {
                _fileLogger?.LogDebug($"Move from e-chessboard is not valid: {move}");
                {
                    ...
                    if (alterNateMove != null)
                    {
                        ...
                        if (!_chessBoard.MoveIsValid(fromField, toField))
                        {
                            _fileLogger?.LogDebug($"First take back failed.");
                            _chessBoard.TakeBack();
                            if (!_chessBoard.MoveIsValid(fromField, toField))
                            {
                                _fileLogger?.LogDebug($"Second take back failed.");
                                if (_eChessBoard != null && _eChessBoard.SelfMoving)
                                {
                                    _eChessBoard?.SetFen(_chessBoard.GetFenPosition());
                                }
            ...
            if (move.Length > 4 && (FigureId.GetFenCharacterToFigureId(move.Substring(4, 1)) != FigureId.NO_PIECE))
            {
            ...
            Dispatcher?.Invoke(() =>
            {
                ...
                if (_showLastMove)
                {
                    chessBoardUcGraphics.MarkLastMoveFields(new[] { fromField, toField });
                }
            });
                ...
                if (prevMove != null)
                {
                    Dispatcher?.Invoke(() =>
                    {
                        if (string.IsNullOrWhiteSpace(promoteFigure))
                            MoveExtentions.SpeakMove(fromFieldFigureId, toFieldFigureId, fromFieldFieldName, toFieldFieldName,
                                FigureId.NO_PIECE, prevMove.GetMove(_chessBoard.EnemyColor).ShortMoveIdentifier, _rm.GetString("YourMove"), false);
                        else
                            MoveExtentions.SpeakMove(fromFieldFigureId, toFieldFigureId, fromFieldFieldName, toFieldFieldName,
                                FigureId.GetFenCharacterToFigureId(promoteFigure),
                                prevMove.GetMove(_chessBoard.EnemyColor).ShortMoveIdentifier, _rm.GetString("YourMove"), false);
                    });
                }
            }
            ...
            if (isInCheck.Equals("#"))
            {
                Dispatcher?.Invoke(() =>
                {
                    ...
                    _eChessBoard?.ShowMove(new SetLEDsParameter()
                                           ...
                                           }, _alternateFenMoveList.Select(a => a.Fen).ToArray());
        ...
        private void LoadAGame(DatabaseGame databaseGame)
        {
            ...
            foreach (var aMove in databaseGame.MoveList)
            {
                ...
                if (aMove.IsEngineMove)
                {
                    _engineWindow?.AddValue(
                        aMove.FigureColor == Fields.COLOR_WHITE ? databaseGame.White : databaseGame.Black, aMove.Score, aMove.FigureColor,
                        bool.Parse(_configuration.GetConfigValue("showForWhite", "false")));
                }

                if (withBuddyEngine)
                {
                    _engineWindow?.AddValue(Guid.Empty.ToString("N"), aMove.ScoreBuddy, aMove.FigureColor, bool.Parse(_configuration.GetConfigValue("showForWhite", "false")));
                }
            ...
            _currentSite = databaseGame.GameSite;
            _currentRound = databaseGame.GameRound;
            ...
            if (_currentGame==null)
            {
                ...
                _currentGame.WhiteConfig.SetElo(databaseGame.WhiteElo);
                _currentGame.BlackConfig.SetElo(databaseGame.BlackElo);
            }
            _currentGame.GameEvent = _currentEvent;
            _currentGame.GameNumber = databaseGame.PgnGame.Round;
            ...
            _moveListWindow?.SetPlayerAndResult(_currentGame, _gameStartFenPosition, databaseGame.PgnGame);
            chessBoardUcGraphics.RepaintBoard(_chessBoard);
            _databaseWindow?.FilterByFen(_chessBoard.GetFenPosition());
            var chessBoard = new ChessBoard();
            chessBoard.Init();
            ...
            if (databaseGame.CurrentGame != null)
            {
                chessBoardUcGraphics.SetPlayer(databaseGame.White, databaseGame.Black,
                    databaseGame.CurrentGame.WhiteConfig.IsPlayer && databaseGame.CurrentGame.BlackConfig.IsPlayer);
            }
            else
            {
                chessBoardUcGraphics.SetPlayer(databaseGame.White, databaseGame.Black, false);
            }
        ...
        private void FillMoveListWindow(ChessBoard aChessBoard, int moveNumberToMark, int colorToMark)
        {
            if (_moveListWindow == null)
            {
                return;
            ...
            chessBoard.NewGame();
            {
                int moveNumber = 0;
                int moveColor = Fields.COLOR_WHITE;
                foreach (var move in aChessBoard.GetPlayedMoveList())
                {
                    if (move == null)
                    {
                        if (moveNumber == 0)
                ...
                _moveListWindow.MarkMove(moveNumberToMark, colorToMark);
            }
            _moveListWindow?.RemainingMovesFor50MovesDraw(aChessBoard.RemainingMovesFor50MovesDraw);
        ...
        {
            if (_currentAction == CurrentAction.InPuzzleMode || _inTrainingMode || _inRepertoireTrainingMode)
            {
                return;
            }
            if (_playedMoveList.Length == 0)
        ...
        private void MoveListWindow_SelectedMoveChanged(object sender, SelectedMoveOfMoveList e)
        {
            ...
            if (_eChessBoard != null && _eChessBoard.SelfMoving)
            {
                _eChessBoard.SetFen(chessBoard.GetFenPosition());
            }
            _databaseWindow?.FilterByFen(chessBoard.GetFenPosition());
            _moveListWindow?.ClearMark();
            _moveListWindow?.RemainingMovesFor50MovesDraw(chessBoard.RemainingMovesFor50MovesDraw);
        ...
        private void TimeOutByClock(string result, string whiteOrBlack, int color)
        {
            ...
            if (_ficsClient != null && _ficsClient.IsLoggedIn)
            {
                Dispatcher?.Invoke(() =>
                {
                    ...
                    _engineWindow?.SetTimeControl(TimeControlHelper.GetAverageTimeControl(), TimeControlHelper.GetAverageTimeControl());
                });
                return;
            }
            _engineWindow?.Stop();
                ...
                Dispatcher?.Invoke(() =>
                {
                    ...
                    
                    {
                        ...
                        if (messageBoxResult == MessageBoxResult.Yes)
                        {
                            ...
                            _chessClocksWindowWhite.CountDown = false;
                            _chessClocksWindowBlack.CountDown = false;
                            _chessClocksWindowWhite.SetTime(0, 0, 0, 0);
                            _chessClocksWindowBlack.SetTime(0, 0, 0, 0);
        ...
        {
            TimeOutByClock("1-0",_rm.GetString("Black"), Fields.COLOR_BLACK);
        }

        private void ChessClocksWindowWhite_TimeOutEvent(object sender, EventArgs e)
        {
            TimeOutByClock("0-1", _rm.GetString("White"), Fields.COLOR_WHITE);
        }
        ...
        private void BearChessMainWindow_OnLoaded(object sender, RoutedEventArgs e)
        {
            ...
            if (_blindUser)
            {
                if (_configuration.GetBoolValue("maximizedWindows", true))
                {
            ...
            if (!_runLastGame)
            {
                ...
                this.Focus();
                if (!_blindUser && _configuration.GetBoolValue("loadLastFenPosition", false))
                {
                    try
                    {
                        if (File.Exists(_savedFenFileName))
                        {
                            SetFenPosition(File.ReadAllText(_savedFenFileName));
                        }
                    }
                    catch
                    {
            ...
            var playerWhite = Constants.Player1;
            var playerBlack = Constants.Player1;
        ...
        private void BearChessMainWindow_OnClosing(object sender, CancelEventArgs e)
        {
            ...
            {
                var fenPosition =_chessBoard.GetFenPosition();
                if (!_chessBoard.IsBasePosition(fenPosition))
                {
                    File.WriteAllText(_savedFenFileName, fenPosition);
                }
            }
            catch
            {
            ...
            else
            {
                ...
                _configuration.SetDoubleValue("ChessBoardUcGraphicsChessBackgroundFieldSizeMDI",
                    chessBoardUcGraphics.ChessBackgroundFieldSize);
                _configuration.SetDoubleValue("ChessBoardUcGraphicsChessFieldSizeMDI",
                    chessBoardUcGraphics.ChessFieldSize);
                _configuration.SetDoubleValue("ChessBoardUcGraphicsControlButtonSizeMDI",
                    chessBoardUcGraphics.ControlButtonSize);
            }

            _configuration.SetBoolValue("maximizedWindows", this.WindowState == WindowState.Maximized);
            _engineWindow?.CloseLogWindow();
            _engineWindow?.Quit();
            _engineWindow?.Close();
            _eChessBoard?.Close();
            _databaseWindow?.Close();
            _puzzleDatabaseWindow?.Close();
            _chessClocksWindowWhite?.Close();
            _chessClocksWindowBlack?.Close();
           ...
           _tournamentManager?.SaveGame(new DatabaseGame(pgnGame, _chessBoard.GetPlayedMoveList(), _currentGame)
                                       {
                                           ...
                                           BlackClockTime = _chessClocksWindowBlack?.GetClockTime(),
                                           Id = _currentGame.RepeatedGame ? _databaseGame.Id : 0
           }, _currentGame.RepeatedGame ? _currentPairing : _tournamentManager.GetPairing());
           SetButtonsForDuelTournament(false);
        ...
        private void HandleEngineTournament()
        {
            ...
            int[] pairing;
            if (_currentGame.Pair1 >= 0)
            {
                pairing = new[] { _currentGame.Pair1, _currentGame.Pair2 };
            }
            else
            {
                pairing = _tournamentManager.GetPairing();
            }
            ...
            _tournamentManager.SaveGame(new DatabaseGame(pgnGame, _chessBoard.GetPlayedMoveList(), _currentGame)
                                        {
                                            ...
                                            BlackClockTime = _chessClocksWindowBlack.GetClockTime(),
                                            Id = _currentGame.RepeatedGame ? _databaseGame.Id : 0
            }, _currentGame.RepeatedGame ? _currentPairing : pairing);
            Dispatcher?.Invoke(() =>
            {
                ...
                {
                    _tournamentInfoWindow?.AddResult(_lastResult, pairing);
                }
        ...
        private void MenuItemLoadEngineTour_OnClick(object sender, RoutedEventArgs e)
        {
            if (_tournamentWindow == null)
            {
                ...
                _tournamentWindow.ParticipantChanged += TournamentWindow_ParticipantChanged;
                _tournamentWindow.Show();
            }
        }

        private void TournamentWindow_ParticipantChanged(object sender, DatabaseTournament e)
        {
            if (_tournamentManager == null)
            {
                _tournamentManager = new TournamentManager(_database);
            ...
            if (!_tournamentManager.ValidForReplace(e.TournamentId))
            {
                BearChessMessageBox.Show(_rm.GetString("TorunamentInvalidForReplace"), _rm.GetString("InvalidForReplace"), MessageBoxButton.OK,
                    MessageBoxImage.Exclamation);
                return;
            }
            var tournamentWindow =
                new ReplaceTournamentParticipantWindow(
                    _usedEngines.Values.Where(v => !v.IsChessComputer && !v.IsInternalBearChessEngine).ToArray(),
                    _configuration, _database, _tournamentManager.Load(e.TournamentId), true);
            var showDialog = tournamentWindow.ShowDialog();
            if (showDialog.HasValue && showDialog.Value)
            {
                if (tournamentWindow.CreateNewTournament)
                {
                    _currentTournamentId = _tournamentManager.CloneTournament(e.TournamentId);
                    _currentTournament = _tournamentManager.Load(_currentTournamentId);
                }
                _tournamentManager.ReplaceParticipant(tournamentWindow.RemovedParticipant,
                    tournamentWindow.AddedParticipant);
            }
        }
        ...
        private void TournamentWindow_RepeatGameSelected(object sender, int gameId)
        {
            ...
            if (_currentTournament != null)
            {
                ...
                if (_currentGame == null)
                {
                    ...
                    if (_currentGame != null)
                    {
                        _database.ResetPairing(_currentTournamentId,_currentPairing);
        ...
        private void MenuItemHelp_OnClick(object sender, RoutedEventArgs e)
        {
            ...
            {
                if (File.Exists("BearChess_DE.pdf"))
                {
                    Process.Start("BearChess_DE.pdf");
                    return;
                }
            }
            BearChessMessageBox.Show(english ? _rm.GetString("BearChessPDFNotFound"): _rm.GetString("BearChessPDFDENotFound"), _rm.GetString("MisingFile"), MessageBoxButton.OK,
                    MessageBoxImage.Error);
        }
        ...
        private void ShowEBoardPosition()
        {
            ...
            if (_sdiLayout)
            {
                ...
                _eBoardControlWindow.SetFen(_eChessBoard.GetBoardFen());
            }
            else
            {
                _eBoardControlWindow = chessControlBoardUserControl;
                Dispatcher?.Invoke(() => { _eBoardControlWindow.SetFen(_eChessBoard.GetBoardFen()); });
            }
        ...
        private void FicsWindow_NewGameEvent(object sender, FicsNewGameInfo e)
        {
            ...
            if (e.PlayerWhite.Equals(ficsUserName))
            {
                ...
                _currentGame = new CurrentGame(_installedEngines[Constants.Player1], _installedEngines[Constants.FICS],
                                               "FICS game",
            ...
            else
            {
                ...
                _currentGame = new CurrentGame(_installedEngines[Constants.FICS], _installedEngines[Constants.Player1],
                                               "FICS game",
                                               timeControl, e.PlayerWhite,
                                               e.PlayerBlack,
                                               true, false)
        ...
        {
            ConnectDisconnectToBoard(ConnectToDGT, DisconnectFromDGT);
        }


        private void MenuItemConfigureDGT_OnClick(object sender, RoutedEventArgs e)
        {
            ConfigureEBoard(Constants.DGT,ConnectToDGT,DisconnectFromDGT,
                new ConfigureDGTWindow(_configuration, _useBluetoothDGT) { Owner = this });
        }
        ...
        {
            ConnectDisconnectToBoard(ConnectToUCB, DisconnectFromUCB);
        }

        private void MenuItemConfigureUCB_OnClick(object sender, RoutedEventArgs e)
        {
            ConfigureEBoard(Constants.UCB,ConnectToUCB,DisconnectFromUCB, new WinConfigureUCB(_configuration) { Owner = this });
        }

        private void MenuItemConnectCitrineBoard_OnClick(object sender, RoutedEventArgs e)
        {
            ConnectDisconnectToBoard(ConnectToCitrine, DisconnectFromCitrine);
        }

        private void MenuItemConfigureCitrine_OnClick(object sender, RoutedEventArgs e)
        {
            ConfigureEBoard(Constants.Citrine,ConnectToCitrine,DisconnectFromCitrine,  new WinConfigureCitrine(_configuration) { Owner = this });
        }

        private void MenuItemConnectToOSA_OnClick(object sender, RoutedEventArgs e)
        {
            ConnectDisconnectToBoard(ConnectToOSA, DisconnectFromOSA);
        }

        private void MenuItemConfigureOSA_OnClick(object sender, RoutedEventArgs e)
        {
            ConfigureEBoard(Constants.OSA,ConnectToOSA,DisconnectFromOSA,  new WinConfigureOSA(_configuration) { Owner = this });
        }
        ...
        private void ConfigureEBoard(string boardName , Action connectToBoard, Action disconnectFromBoard, Window configDialog )
        {
            if (_currentAction == CurrentAction.InRunningGame)
            {
                return;
            ...
            var showDialog =  configDialog.ShowDialog();
            if (showDialog.HasValue && showDialog.Value)
            {
                _lastEBoard = boardName;
                textBlockButtonConnect.Text = _lastEBoard;
        ...
        private void ConnectDisconnectToBoard(Action connectToBoard, Action disconnectFromBoard)
        {
            if (_currentAction == CurrentAction.InRunningGame)
            {
                return;
        ...
        {
            ConnectDisconnectToBoard(ConnectToTabutronicSentio, DisconnectFromTabutronicSentio);
        }

        private void MenuItemConnectSentioSpectrum_OnClick(object sender, RoutedEventArgs e)
        {
            ConnectDisconnectToBoard(ConnectToTabutronicSentioSpectrum, DisconnectFromTabutronicSentioSpectrum);
        }

        private void MenuItemConfigureSentio_OnClick(object sender, RoutedEventArgs e)
        {
            ConfigureEBoard(Constants.TabutronicSentio,ConnectToTabutronicSentio,DisconnectFromTabutronicSentio, new ConfigureSentioWindow(_configuration, _useBluetoothTabuTronicSentio,
                _useBluetoothLETabuTronicSentio) { Owner = this });

        }

        private void MenuItemConfigureSentioSpectrum_OnClick(object sender, RoutedEventArgs e)
        {
            ConfigureEBoard(Constants.TabutronicSentioSpectrum, ConnectToTabutronicSentioSpectrum,
                DisconnectFromTabutronicSentioSpectrum,
                new ConfigureSentioSpectrumWindow(_configuration, _useBluetoothTabuTronicSentioSpectrum,
                    _useBluetoothLETabuTronicSentioSpectrum) { Owner = this });
        ...
        private void MenuItemBluetoothSentioSpectrum_OnClick(object sender, RoutedEventArgs e)
        {
            _useBluetoothTabuTronicSentioSpectrum = !_useBluetoothTabuTronicSentioSpectrum;
            _configuration.SetConfigValue("useBluetoothTabuTronicSentioSpectrum", _useBluetoothTabuTronicSentioSpectrum.ToString());
            imageSentioSpectrumBluetooth.Visibility = _useBluetoothTabuTronicSentioSpectrum ? Visibility.Visible : Visibility.Hidden;
            _useBluetoothLETabuTronicSentioSpectrum = false;
            _configuration.SetConfigValue("usebluetoothLETabuTronicSentioSpectrum", _useBluetoothLETabuTronicCernoSpectrum.ToString());
            imageSentioSpectrumBluetoothLE.Visibility = Visibility.Hidden;
        }

        private void MenuItemConnectCerno_OnClick(object sender, RoutedEventArgs e)
        {
            ConnectDisconnectToBoard(ConnectToTabutronicCerno,DisconnectFromTabutronicCerno);
        }

        private void MenuItemConnectCernoSpectrum_OnClick(object sender, RoutedEventArgs e)
        {
            ConnectDisconnectToBoard(ConnectToTabutronicCernoSpectrum, DisconnectFromTabutronicCernoSpectrum);
        }

        private void MenuItemConfigureCernoSpectrum_OnClick(object sender, RoutedEventArgs e)
        {
            ConfigureEBoard(Constants.TabutronicCernoSpectrum, ConnectToTabutronicCernoSpectrum,
                DisconnectFromTabutronicCernoSpectrum,
                new ConfigureCernoSpectrumWindow(_configuration, _useBluetoothTabuTronicCernoSpectrum,
                    _useBluetoothLETabuTronicCernoSpectrum) { Owner = this });
        }


        private void MenuItemConfigureCerno_OnClick(object sender, RoutedEventArgs e)
        {
            ConfigureEBoard(Constants.TabutronicCerno, ConnectToTabutronicCerno,
                DisconnectFromTabutronicCerno,
                new ConfigureCernoWindow(_configuration, _useBluetoothTabuTronicCerno, _useBluetoothLETabuTronicCerno) { Owner = this });
        }
        ...
        private void MenuItemLoadLastOrDefaultBook_OnClick(object sender, RoutedEventArgs e)
        {
            _loadLastOrDefaultBook = !_loadLastOrDefaultBook;
            _configuration.SetBoolValue("LoadLastOrDefaultBook", _loadLastOrDefaultBook);
            imageLoadLastOrDefaultBook.Visibility = _loadLastOrDefaultBook ? Visibility.Visible : Visibility.Hidden;
        }
        ...
        {
            ConnectDisconnectToBoard(ConnectToIChessOne, DisconnectFromIChessOne);
        }
        ...
        {
            ConfigureEBoard(Constants.IChessOne, ConnectToIChessOne,
                DisconnectFromIChessOne,
                new ConfigureIChessOneWindow(_configuration,_useBluetoothIChessOne) { Owner = this });
        }


        private void MenuItemConfigureSquareOff_OnClick(object sender, RoutedEventArgs e)
        {
            ConfigureEBoard(Constants.SquareOffPro, ConnectToSquareOffPro,
                DisconnectFromSquareOffPro,
                new ConfigureSquareOffWindow(_configuration) { Owner = this });
        }
        ...
        private void MenuItemConfigBearChess_OnClick(object sender, RoutedEventArgs e)
        {
            ...
            var tournamentMode =_configuration.GetBoolValue("tournamentMode", false);
            ...
            if (showDialog.HasValue && showDialog.Value)
            {
                ...
                var tournamentModeNew = _configuration.GetBoolValue("tournamentMode", false);
                if (!configValueLanguage.Equals(configValueLanguageNew) || !sdiLayout.Equals(sdiLayoutNew)
                                                                        || _configuration.ClearLogIsSet()
                                                                        || !tournamentMode.Equals(tournamentModeNew)
        ...
        private void MenuItemConfigureEBoard_OnClick(object sender, RoutedEventArgs e)
        {
            ...
            if (_lastEBoard.Equals(Constants.ChessnutGo))
            {
                MenuItemConfigureChessnutGo_OnClick(sender, e);
                return;
            }
            if (_lastEBoard.Equals(Constants.ChessnutMove))
            {
                MenuItemConfigureChessnutMove_OnClick(sender, e);
                return;
            }
            ...
            if (_lastEBoard.Equals(Constants.TabutronicSentioSpectrum))
            {
                MenuItemConfigureSentioSpectrum_OnClick(sender, e);
                return;
            }
        ...
        {
            ConnectDisconnectToBoard(ConnectToChessUp, DisconnectFromChessUp);
        }
        ...
        private void DisableConnectItems()
        {
            ...
            menuItemChessnutMoveBoard.IsEnabled = false;
        ...
        private void MenuItemConfigureChessnutGo_OnClick(object sender, RoutedEventArgs e)
        {
            ...
            var winConfigureChessnut = new ConfigureChessnutWindow(Constants.ChessnutGo, _configuration, _useBluetoothChessnutGo) { Owner = this };
            var showDialog = winConfigureChessnut.ShowDialog();
            if (showDialog.HasValue && showDialog.Value)
            {
        ...
        private void MenuItemBluetoothLECernoSpectrum_OnClick(object sender, RoutedEventArgs e)
        {
            ...
            _configuration.SetBoolValue("usebluetoothTabuTronicCernoSpectrum", _useBluetoothTabuTronicCernoSpectrum);
            imageCernoSpectrumBluetooth.Visibility = Visibility.Hidden;
        }
        ...
        private void MenuItemBluetoothLESentioSpectrum_OnClick(object sender, RoutedEventArgs e)
        {
            _useBluetoothLETabuTronicSentioSpectrum = !_useBluetoothLETabuTronicSentioSpectrum;
            _configuration.SetConfigValue("usebluetoothLETabuTronicSentioSpectrum", _useBluetoothLETabuTronicSentioSpectrum.ToString());
            imageSentioSpectrumBluetoothLE.Visibility = _useBluetoothLETabuTronicSentioSpectrum ? Visibility.Visible : Visibility.Hidden;
            _useBluetoothTabuTronicSentioSpectrum = false;
            _configuration.SetConfigValue("usebluetoothTabuTronicSentioSpectrum", _useBluetoothTabuTronicSentioSpectrum.ToString());
            imageSentioSpectrumBluetooth.Visibility = Visibility.Hidden;
        }
        ...
        private void ConnectToHoS()
        {
            ...
            _clockUseExtern = _eChessBoard.GetEChessBoardConfiguration().UseClock && (_eChessBoard.GetEChessBoardConfiguration().ClockUseExtern || _tournamentMode);
            _clockUseExtern = _eChessBoard.GetEChessBoardConfiguration().UseClock && _eChessBoard.GetEChessBoardConfiguration().ClockUseExtern;
            if (_chessClocksWindowWhite != null)
            {
                _chessClocksWindowWhite.SimpleClockMode = _clockUseExtern;
            }
        ...
        private void EChessBoard_ClockEvent(object sender, ClockFromBoard e)
        {
            Dispatcher?.Invoke(() =>
            {
                if (_eChessBoard.Configuration.ClockUseExtern)
                {
                    if (e.IsRunning)
                    {
                       //if (e.LeftIsRunning)
        ...
        private void MenuItemConnectHoSBoard_OnClick(object sender, RoutedEventArgs e)
        {
            ...
            buttonShowEBoardPosition.Visibility = _eChessBoard != null ? Visibility.Hidden : Visibility.Visible;
            ConnectDisconnectToBoard(ConnectToHoS, DisconnectFromHoS);
        }
        ...
        {
            ConnectDisconnectToBoard(ConnectToChessnutEvo, DisconnectFromChessnutEvo);
        }
        ...
        private void BearChessMainWindow_OnPreviewKeyDown(object sender, KeyEventArgs e)
        {
            if (_blindUser)
            {
        ...
        private void BearChessMainWindow_OnKeyDown(object sender, KeyEventArgs e)
        {
            if (_blindUser || _speechIsActive)
            {
                if (e.Key == Key.F1)
                {
                    ...
                    {
                        _synthesizer?.SpeakAsync(_currentHelpText, true);
                        return;
                    }
                    ...
                    if (_getSaysForBoard.Length > 0)
                    {
                        ...
                        _blindQueryFieldNumber = string.Empty;
                    }
                    else
                    {
                        ...
                        _getSaysForBoardIndex = 0;
                        _synthesizer?.SpeakAsync(_rm.GetString("HelpRequestedActivated"));
                        _synthesizer?.SpeakAsync(_getSaysForBoard[_getSaysForBoardIndex], true);
                    }
                    return;
                }
                ...
                if (_blindUser)
                {
                    ...
                    if ((e.Key == Key.NumPad1 || e.Key==Key.D1) && _getSaysForBoard.Length > 0)
                    {
                        _blindQueryFieldNumber += "1";
                    }
                    if ((e.Key == Key.NumPad2 || e.Key == Key.D2) && _getSaysForBoard.Length > 0)
                    {
                        _blindQueryFieldNumber += "2";
                    }
                    if ((e.Key == Key.NumPad3 || e.Key == Key.D3) && _getSaysForBoard.Length > 0)
                    {
                        _blindQueryFieldNumber += "3";
                    }
                    if ((e.Key == Key.NumPad4 || e.Key == Key.D4) && _getSaysForBoard.Length > 0)
                    {
                        _blindQueryFieldNumber += "4";
                    }
                    if ((e.Key == Key.NumPad5 || e.Key == Key.D5) && _getSaysForBoard.Length > 0)
                    {
                        _blindQueryFieldNumber += "5";
                    }
                    if ((e.Key == Key.NumPad6 || e.Key == Key.D6) && _getSaysForBoard.Length > 0)
                    {
                        _blindQueryFieldNumber += "6";
                    }
                    if ((e.Key == Key.NumPad7 || e.Key == Key.D7) && _getSaysForBoard.Length > 0)
                    {
                        _blindQueryFieldNumber += "7";
                    }
                    if ((e.Key == Key.NumPad8 || e.Key == Key.D8) && _getSaysForBoard.Length > 0)
                    {
                        _blindQueryFieldNumber += "8";
                    }
                    if (e.Key == Key.A && _getSaysForBoard.Length > 0)
                    {
                        _blindQueryFieldNumber += "A";
                    }
                    if (e.Key == Key.B && _getSaysForBoard.Length > 0)
                    {
                        _blindQueryFieldNumber += "B";
                    }
                    if (e.Key == Key.C && _getSaysForBoard.Length > 0)
                    {
                        _blindQueryFieldNumber += "C";
                    }
                    if (e.Key == Key.D && _getSaysForBoard.Length > 0)
                    {
                        _blindQueryFieldNumber += "D";
                    }
                    if (e.Key == Key.E && _getSaysForBoard.Length > 0)
                    {
                        _blindQueryFieldNumber += "E";
                    }
                    if (e.Key == Key.F && _getSaysForBoard.Length > 0)
                    {
                        _blindQueryFieldNumber += "F";
                    }
                    if (e.Key == Key.G && _getSaysForBoard.Length > 0)
                    {
                        _blindQueryFieldNumber += "G";
                    }
                    if (e.Key == Key.H && _getSaysForBoard.Length > 0)
                    {
                        _blindQueryFieldNumber += "H";
                    }

                    if (_blindQueryFieldNumber.Length == 2)
                    {
                        var fieldNumber = Fields.COLOR_EMPTY;
                        if (int.TryParse(_blindQueryFieldNumber, out _))
                        {
                            fieldNumber = Fields.GetBlindFieldNumber(_blindQueryFieldNumber);
                        }
                        else
                        {
                            fieldNumber = Fields.GetFieldNumber(_blindQueryFieldNumber);
                        }

                        if (fieldNumber == Fields.COLOR_EMPTY)
                        {
                            _blindQueryFieldNumber = string.Empty;
                            _synthesizer?.SpeakAsync($"{_rm.GetString("Invalid")} {_blindQueryFieldNumber}", true);
                            return;
                        }
                        var fieldName = Fields.GetFieldName(fieldNumber);
                        var chessFigure = _chessBoard.GetFigureOn(fieldNumber); 
                        var speechLanguageTag = Configuration.Instance
                            .GetConfigValue("selectedSpeechCulture", CultureInfo.CurrentCulture.IetfLanguageTag).ToLower();
                        var figureName = SpeechTranslator.GetFigureName(chessFigure.FigureId, speechLanguageTag, Configuration.Instance);
                        if (string.IsNullOrEmpty(figureName))
                        {
                            figureName = $"{fieldName} {_rm.GetString("NoFigure")}";
                        }
                        else
                        {
                            var figureColor = chessFigure.Color == Fields.COLOR_WHITE ? "White" : "Black";
                            figureName = $"{fieldName} {_rm.GetString(figureColor)}  {figureName}";
                        }
                        _synthesizer?.SpeakAsync(figureName, true);
                        _blindQueryFieldNumber = string.Empty;
                    }
                    ...
                    if (e.Key == Key.A && _currentAction == CurrentAction.InRunningGame && _blindUserNoBoard)
                    {
                        _blindMoveFieldInput += 'A';
                    }

                    if (e.Key == Key.B) 
                    {
                        if (_currentAction == CurrentAction.InRunningGame && _blindUserNoBoard)
                        {
                            _blindMoveFieldInput += 'B';
                    ...
                    if (e.Key == Key.C && _currentAction == CurrentAction.InRunningGame && _blindUserNoBoard)
                    {
                        _blindMoveFieldInput += 'C';
                    }
                    if (e.Key == Key.D && _currentAction == CurrentAction.InRunningGame && _blindUserNoBoard)
                    {
                        _blindMoveFieldInput += 'D';
                    }
                    if (e.Key == Key.E && _currentAction == CurrentAction.InRunningGame && _blindUserNoBoard)
                    {
                        _blindMoveFieldInput += 'E';
                    }
                    if (e.Key == Key.F && _currentAction == CurrentAction.InRunningGame && _blindUserNoBoard)
                    {
                        _blindMoveFieldInput += 'F';
                    }
                    if (e.Key == Key.G && _currentAction == CurrentAction.InRunningGame && _blindUserNoBoard)
                    {
                        _blindMoveFieldInput += 'G';
                    }
                    if (e.Key == Key.H && _currentAction == CurrentAction.InRunningGame && _blindUserNoBoard)
                    {
                        _blindMoveFieldInput += 'H';
                    }
                    if ((e.Key == Key.NumPad1 || e.Key == Key.D1) && _currentAction == CurrentAction.InRunningGame && _blindUserNoBoard)
                    {
                        _blindMoveFieldInput += "1";
                    }
                    if ((e.Key == Key.NumPad2 || e.Key == Key.D2) && _currentAction == CurrentAction.InRunningGame && _blindUserNoBoard)
                    {
                        _blindMoveFieldInput += "2";
                    }
                    if ((e.Key == Key.NumPad3 || e.Key == Key.D3) && _currentAction == CurrentAction.InRunningGame && _blindUserNoBoard)
                    {
                        _blindMoveFieldInput += "3";
                    }
                    if ((e.Key == Key.NumPad4 || e.Key == Key.D4) && _currentAction == CurrentAction.InRunningGame && _blindUserNoBoard)
                    {
                        _blindMoveFieldInput += "4";
                    }
                    if ((e.Key == Key.NumPad5 || e.Key == Key.D5) && _currentAction == CurrentAction.InRunningGame && _blindUserNoBoard)
                    {
                        _blindMoveFieldInput += "5";
                    }
                    if ((e.Key == Key.NumPad6 || e.Key == Key.D6) && _currentAction == CurrentAction.InRunningGame && _blindUserNoBoard)
                    {
                        _blindMoveFieldInput += "6";
                    }
                    if ((e.Key == Key.NumPad7 || e.Key == Key.D7) && _currentAction == CurrentAction.InRunningGame && _blindUserNoBoard)
                    {
                        _blindMoveFieldInput += "7";
                    }
                    if ((e.Key == Key.NumPad8 || e.Key == Key.D8) && _currentAction == CurrentAction.InRunningGame && _blindUserNoBoard)
                    {
                        _blindMoveFieldInput += "8";
                    }
                    if (_blindMoveFieldInput.Length == 4)
                    {
                        int fromFieldNumber = 0;
                        int toFieldNumber = 0;
                        string fromField = _blindMoveFieldInput.Substring(0, 2);
                        string toField = _blindMoveFieldInput.Substring(2, 2);
                        if (int.TryParse(fromField, out int _))
                        {
                            fromFieldNumber = Fields.GetBlindFieldNumber(fromField);
                        }
                        else
                        {
                            fromFieldNumber = Fields.GetFieldNumber(fromField);
                        }
                        if (int.TryParse(toField, out int _))
                        {
                            toFieldNumber = Fields.GetBlindFieldNumber(toField);
                        }
                        else
                        {
                            toFieldNumber = Fields.GetFieldNumber(toField);
                        }

                        _blindMoveFieldInput = string.Empty;
                        chessBoardUcGraphics.MakeMove(fromFieldNumber,toFieldNumber);
                        return;
                    ...
                    if (e.Key == Key.Z)
                    {
                        if (_currentAction == CurrentAction.InRunningGame &&  _blindUserNoBoard)
                        {
                            if ((_currentGame.WhiteConfig.IsPlayer && _chessBoard.CurrentColor == Fields.COLOR_WHITE)
                                || (_currentGame.BlackConfig.IsPlayer && _chessBoard.CurrentColor == Fields.COLOR_BLACK))
                            {
                                ChessBoardUcGraphics_TakeStepBackEvent(this, null);
                            }
                        }
                        return;
                    }
        ...
        {
            ConnectDisconnectToBoard(ConnectToTabutronicTactum, DisconnectFromTabutronicTactum);
        ...
        private void _bearChessServerClient_ServerMessage(object sender, BearChessServerMessage e)
        {
            ...
            {
                _bcsPlayerWhite = string.Empty;
                _bcsPlayerBlack = string.Empty;
                _bcsOwnColor = Fields.COLOR_EMPTY;
                _bcsTournament = string.Empty;
                Dispatcher?.Invoke(() =>
                {
            ...
            if (e.ActionCode.Equals(BCServerConstants.ActionTournament))
            {
                _bcsPlayerWhite = string.Empty;
                _bcsPlayerBlack = string.Empty;
                _bcsTournament = e.Message;
            }
            if (e.ActionCode.Equals(BCServerConstants.ActionPlayerWhite))
            {
                _bcsPlayerWhite = e.Message;
                if (e.Color.Equals(e.Address))
                {
                    _bcsOwnColor = Fields.COLOR_WHITE;
                }
            }
            if (e.ActionCode.Equals(BCServerConstants.ActionPlayerBlack))
            {
                _bcsPlayerBlack = e.Message;
                if (e.Color.Equals(e.Address))
                {
                    _bcsOwnColor = Fields.COLOR_BLACK;
                }
            }
        }
        ...
        private void _bearChessServerClient_DisConnected(object sender, Exception e)
        {
            Dispatcher?.Invoke(() =>
            {
                ...
                if (e != null)
                {
                    BearChessMessageBox.Show(e.Message, "Connection failed", MessageBoxButton.OK,
                        MessageBoxImage.Error);
                }
            });
        }
        ...
        private void MenuItemCopyPosition_OnClick(object sender, RoutedEventArgs e)
        {
            ClipboardHelper.SetText(_chessBoard.GetFenPosition());
        }

        private void SetFenPosition(string fenPosition)
        {
            var currentPosition = _chessBoard.GetFenPosition();

            var positionChanged = !currentPosition.Equals(fenPosition);
        ...
        private void MenuItemPastePosition_OnClick(object sender, RoutedEventArgs e)
        {
            SetFenPosition(ClipboardHelper.GetText());
        }

        private void MenuItemloadLastFen_OnClick(object sender, RoutedEventArgs e)
        {
            var loadLast =  !_configuration.GetBoolValue("loadLastFenPosition", false);
            _configuration.SetBoolValue("loadLastFenPosition", loadLast);
            imageLoadLastFen.Visibility = loadLast ? Visibility.Visible : Visibility.Hidden;
        }

        private void MenuItemloadLastFen_OnKeyDown(object sender, KeyEventArgs e)
        {
            if (e.Key == Key.Space)
            {
                MenuItemloadLastFen_OnClick(sender, e);
            }
        }

        private void MenuItemTrainOpening_OnClick(object sender, RoutedEventArgs e)
        {
            if (_inTrainingMode)
            {
                _trainingFenCodes.Clear();
                _currentAction = CurrentAction.InEasyPlayingMode;
                _inTrainingMode = false;
                _inRepertoireTrainingMode = false;
                menuItemSetupPosition.IsEnabled = true;
                menuItemAnalyseMode.IsEnabled = true;
                menuItemEngineMatch.IsEnabled = true;
                menuItemEngineTour.IsEnabled = true;
                ...
                menuItemNewGame.IsEnabled = true;
                menuItemEloEstimateEngineMatch.IsEnabled = true;
                menuItemStartPuzzle.IsEnabled = true;
                menuItemTrainStartOpening.Visibility = Visibility.Visible;
                menuItemTrainStopOpening.Visibility = Visibility.Collapsed;
                menuItemTrainStartRepertoire.IsEnabled = true;
                menuItemTrainStopRepertoire.IsEnabled = true;
                textBlockRunningMode.Text = _rm.GetString("ModeEasyPlaying");
                _trainingBookInfo = null;
                _trainingEcoCode = null;
                _trainingOpeningBook.Dispose();
                _trainingOpeningBook = null;
                while (_openingTrainingMoves.TryDequeue(out _)) { ; }
                try
                {
            ...
            if (_eChessBoard == null)
            {
                BearChessMessageBox.Show(_rm.GetString("ConnectFirst"), _rm.GetString("ElectronicBoards"), MessageBoxButton.OK,
                    MessageBoxImage.Stop);
                return;
            }
            if (_eChessBoard != null && !_eChessBoard.ValidForTraining)
            {
                BearChessMessageBox.Show(_rm.GetString("BoardNotSuitableForOpening"), _rm.GetString("NotSuitable"),
                    MessageBoxButton.OK,
                    MessageBoxImage.Error);
                return;
            }
            _openingTrainingConfig = _configuration.LoadOpeningTrainingConfig();
            var dialogWindow = new OpeningTrainingStartWindow(OpeningBookLoader.GetInstalledBookInfos(), _ecoCodes.Values.ToArray(),
                _openingTrainingConfig)
                {
                    Owner = this
                };
            var showDialog = dialogWindow.ShowDialog();
            if (showDialog.HasValue && showDialog.Value)
            {
                _openingTrainingConfig = dialogWindow.Config;
                _configuration.SaveOpeningTrainingConfig(_openingTrainingConfig);
                _eChessBoard?.SetAllLEDsOff(true);
              
                _trainingBookInfo = dialogWindow.SelectedBook;
                _trainingEcoCode = dialogWindow.SelectedCode;
                _trainingDatabaseName = dialogWindow.SelectedDatabaseName;
                _chessBoard.NewGame();
                _chessBoard.SetPosition(_trainingEcoCode.FenCode, false);
                _chessBoard.GenerateMoveList();
                chessBoardUcGraphics.RepaintBoard(_chessBoard);
                _currentAction = CurrentAction.InTrainingSetupMode;
                var eboardFen =_eChessBoard?.GetBoardFen();
                if (!_trainingEcoCode.FenCode.StartsWith(eboardFen) || string.IsNullOrEmpty(eboardFen)  )
                {
                    if (_eChessBoard.ValidForAnalyse)
                    {
                        if (_eChessBoard.SelfMoving)
                        {
                            _eChessBoard.SetFen(_trainingEcoCode.FenCode);
                        }
                        ...
                        progressWorker.DoWorkWithModal(progress =>
                        {
                            var eboardFen2 = _eChessBoard?.GetBoardFen();
                            while (!splashProgressControlContents[0].Cancel &&
                                   !_trainingEcoCode.FenCode.StartsWith(eboardFen2) ||
                                   string.IsNullOrEmpty(eboardFen2))
                            {
                                eboardFen2 = _eChessBoard?.GetBoardFen();
                                Thread.Sleep(300);
                ...
                _eChessBoard?.SetCurrentColor(_chessBoard.CurrentColor);
                _currentGame = null;
                menuItemTrainStartOpening.Visibility = Visibility.Collapsed;
                menuItemTrainStopOpening.Visibility = Visibility.Visible;
                menuItemTrainStartRepertoire.IsEnabled = true;
                menuItemTrainStopRepertoire.IsEnabled = true;
                _currentAction = CurrentAction.InEasyPlayingMode;
                _currentWhitePlayer = string.Empty;
                ...
                _playersColor[Fields.COLOR_WHITE] = true;
                _playersColor[Fields.COLOR_BLACK] = true;
                textBlockRunningMode.Text = _rm.GetString("ModeTrainingOpening");
                menuItemSetupPosition.IsEnabled = false;
                menuItemAnalyseMode.IsEnabled = false;
                menuItemEngineMatch.IsEnabled = false;
                menuItemEngineTour.IsEnabled = false;
                ...
                menuItemStartPuzzle.IsEnabled = false;
                menuItemEloEstimateEngineMatch.IsEnabled = false;
                _prevTrainingFenPosition = string.Empty;
                _trainingOpeningBook = new OpeningBook();
                _trainingOpeningBook.LoadBook(_trainingBookInfo.FileName, false);
                _trainingThread= new Thread(HandleOpeningTraining) { IsBackground = true };
                _trainingThread.Start();
                HandleOpeningTraining(_chessBoard.GetFenPosition(), _prevFenPosition);
            }
        }

        private void HandleOpeningTraining()
        {
            try
            {
                int index = 0;
                IBookMoveBase bookMove = null;
                List<IBookMoveBase> allTrainingMoves = new List<IBookMoveBase>();
                while (true)
                {
                    if (!_openingTrainingConfig.ShowNextMove)
                    {
                        allTrainingMoves.Clear();
                        index = 0;
                        while (_openingTrainingMoves.TryDequeue(out _)) { ; }
                    ...
                    allTrainingMoves = new List<IBookMoveBase>(_openingTrainingMoves.ToArray());
                    
                    index++;
                    if (index >= allTrainingMoves.Count)
                    {
                        index = 0;
                    }
                    bookMove = null;
                    if (index < allTrainingMoves.Count)
                    {
                        bookMove = allTrainingMoves[index];
                    }
                    if (bookMove!=null)
                    {
                        var bookMoveFenPosition = bookMove.FenPosition;
                        if (bookMoveFenPosition != null)
                        {
                            if (_ecoCodes.ContainsKey(bookMoveFenPosition))
                            {
                                var ecoCode = _ecoCodes[bookMoveFenPosition];
                                if (ecoCode.Code.Equals(_trainingEcoCode.Code) || _openingTrainingConfig.AllowChangeEcoCode)
                                {
                                    _eChessBoard?.SetLEDsFor(new SetLEDsParameter()
                                    {
                                        FieldNames = new string[] { bookMove.FromField, bookMove.ToField },
                                        BookFieldNames = new string[] { bookMove.FromField, bookMove.ToField },
                                        //FenString = bookMoveFenPosition
                                    });
                                }
                            }
                            else
                            {
                                if (_openingTrainingConfig.AllowChangeEcoCode)
                                {
                                    _eChessBoard?.SetLEDsFor(new SetLEDsParameter()
                                    {
                                        FieldNames = new string[] { bookMove.FromField, bookMove.ToField },
                    ...
                    Thread.Sleep(_openingTrainingConfig.NextMoveSeconds * 1000);
                }
            }
            catch (Exception ex)
            {
         ...
         private void HandleRepertoireOpeningTraining()
        {
            try
            {
                int index = 0;
                IMove gameMove = null;
                List<Move> allTrainingMoves = new List<Move>();
                while (true)
                {
                    if (!_repertoireTrainingConfig.ShowNextMove)
                    {
                        allTrainingMoves.Clear();
                        index = 0;
                        while (_repertoireTrainingMoves.TryDequeue(out _)) { ; }
                        Thread.Sleep(100);
                    }

                    allTrainingMoves = new List<Move>(_repertoireTrainingMoves.ToArray());
                    ...
                    Move trainingMove = null;
                    if (index < allTrainingMoves.Count)
                    {
                        trainingMove = allTrainingMoves[index];
                    }

                    if (trainingMove != null)
                    {
                        _eChessBoard?.SetLEDsFor(new SetLEDsParameter()
                        {
                            FieldNames = new string[] { trainingMove.FromFieldName, trainingMove.ToFieldName },
                            BookFieldNames = new string[] { trainingMove.FromFieldName, trainingMove.ToFieldName },
                        });

                    }

                    Thread.Sleep(_repertoireTrainingConfig.NextMoveSeconds * 1000);
                }
            }
            catch (Exception ex)
            {
        ...
        private void HandleRepertoireOpeningTraining(string fenPosition, string prevFenPosition)
        {
            if (_currentDatabaseRepertoire == null)
            {
                return;
            }
            if (fenPosition.Equals(_prevTrainingFenPosition))
            {
                return;
            }
            _prevTrainingFenPosition = fenPosition;
            while (_repertoireTrainingMoves.TryDequeue(out _)) {; }
            var databaseMovesAvailable = false;
            if (!string.IsNullOrWhiteSpace(prevFenPosition))
            {
                var moveBases = _currentDatabaseRepertoire.GetNextGames(prevFenPosition);
                databaseMovesAvailable = moveBases.Length > 0;
                _fileLogger?.LogDebug($"Repertoire Training: Games/Moves from prev fen {prevFenPosition} available: {databaseMovesAvailable}");
            }
            var nextGames = _currentDatabaseRepertoire.GetNextGames(fenPosition);
            if (nextGames.Length == 0)
            {
                if (databaseMovesAvailable)
                {
                    _fileLogger?.LogDebug($"Repertoire Training: No game/move for this fen {fenPosition} but there is another one possible");
                    if (_chessBoard.CurrentColor == Fields.COLOR_BLACK)
                    {
                        _eChessBoard?.SetLEDsFor(new SetLEDsParameter()
                        {
                            IsError = true,
                            FieldNames = new[] { "a1", "h1" },
                            InvalidFieldNames = new[] { "a1", "h1" }
                        });
                    }
                    else
                    {
                        ...
                        {
                            IsError = true,
                            FieldNames = new[] { "a8", "h8" },
                            InvalidFieldNames = new[] { "a8", "h8" }
                        });
                    }
                }
                else
                {
                    _fileLogger?.LogDebug("Repertoire Training: No more games/moves");
                    _eChessBoard?.SetLEDsFor(new SetLEDsParameter()
                    {
                        BookFieldNames = new[] { "a1", "a8", "h1", "h8" },
                        FieldNames = new[] { "a1", "a8", "h1", "h8" }
                    });
                }
                return;
            }
            var gamesIndex = new Random().Next(nextGames.Length);
            var nextGame = nextGames[gamesIndex];
            var bestMove = nextGame.Game.AllMoves[nextGame.NextMoveIndex];
            var figureOn = _chessBoard.GetFigureOn(bestMove.FromField);
            if (_repertoireTrainingConfig.ShowCurrentGame && _moveListWindow != null)
            {
                var aChessBoard = new ChessBoard();
                aChessBoard.Init();
                aChessBoard.NewGame();
                foreach (var gameAllMove in nextGame.Game.AllMoves)
                {
                    aChessBoard.MakeMove(gameAllMove);
                }
                int markMoveIndex = nextGame.NextMoveIndex / 2;
                if (figureOn.Color == Fields.COLOR_WHITE)
                {
                    FillMoveListWindow(aChessBoard, markMoveIndex, Fields.COLOR_BLACK);
                }
                else
                {
                    FillMoveListWindow(aChessBoard, markMoveIndex + 1, Fields.COLOR_WHITE);
                }
            }
            if (_repertoireTrainingConfig.ExecuteMoveAutomatically &&
                _repertoireTrainingConfig.ExecuteForColor == figureOn.Color)
            {
                _fileLogger.LogDebug($"Repertoire Training: Find next repertoire move to automatically respond");
                _fileLogger?.LogDebug(
                    $"Repertoire Training: set LEDs for execute best repertoire move {bestMove.FromFieldName}{bestMove.ToFieldName} ");
                _eChessBoard?.SetLEDsFor(new SetLEDsParameter()
                {
                    FieldNames = new[] { bestMove.FromFieldName, bestMove.ToFieldName },
                    Promote = string.Empty,
                    IsMove = true,
                    IsEngineMove = true,
                    FenString = bestMove.Fen
                });
            ...
            foreach (var nextRepGame in nextGames)
            {
                var bestGameMove = nextRepGame.Game.AllMoves[nextRepGame.NextMoveIndex];
                if (!uniqueMoves.Contains($"{bestGameMove.FromFieldName}{bestGameMove.ToFieldName}"))
                {
                    _fileLogger?.LogDebug(
                        $"Repertoire Training: enqueue possible repertoire move {bestGameMove.FromFieldName}{bestGameMove.ToFieldName} ");
                    _repertoireTrainingMoves.Enqueue(bestGameMove);
                    uniqueMoves.Add($"{bestGameMove.FromFieldName}{bestGameMove.ToFieldName}");
                }
            }
        }

        
        private void HandleOpeningTraining(string fenPosition, string prevFenPosition)
        {
            if (_trainingOpeningBook == null || _eChessBoard == null)
            {
                return;
            }
            ...
            _prevTrainingFenPosition = fenPosition;
            while (_openingTrainingMoves.TryDequeue(out _)) { ; }
            var bookMovesAvailable = false;
            if (!string.IsNullOrWhiteSpace(prevFenPosition))
            {
                var moveBases = _trainingOpeningBook.GetMoveList(prevFenPosition);
                bookMovesAvailable = moveBases.Length > 0;
                if (bookMovesAvailable)
                {
                    _fileLogger?.LogDebug($"Training: Book moves from prev fen {prevFenPosition}");
                }
                foreach (var bookMoveBase in moveBases)
                {
                    _fileLogger?.LogDebug($"Training: Prev book move: {bookMoveBase.FromField}{bookMoveBase.ToField}");
                }
            }

            IBookMoveBase[] bookMoveBases = _openingTrainingConfig.AllowChangeEcoCode ?
                _trainingOpeningBook.GetMoveList(fenPosition) :
                _trainingOpeningBook.GetMoveList(fenPosition, _ecoCodes, _trainingEcoCode.Code);
            if (bookMoveBases.Length == 0)
            {
                if (bookMovesAvailable)
                {
                    _fileLogger?.LogDebug("Training: No book moves but there was one");
                    _eChessBoard?.SetLEDsFor(new SetLEDsParameter()
                    {
                        IsError = true,
                        FieldNames = new[] { "a1", "a8", "h1", "h8" },
                        InvalidFieldNames = new[] { "a1", "a8", "h1", "h8" }
                    });
                }
                else
                {
                    _fileLogger?.LogDebug("Training: No book moves");
                    _eChessBoard?.SetLEDsFor(new SetLEDsParameter()
                    {
                        BookFieldNames = new[] { "a1", "a8", "h1", "h8" },
                        FieldNames = new[] { "a1", "a8", "h1", "h8" }
            ...
            var bestBookMove = bookMoveBases[0];
            var figureOn = _chessBoard.GetFigureOn(Fields.GetFieldNumber(bestBookMove.FromField));
            if (_openingTrainingConfig.ExecuteMoveAutomatically &&
                _openingTrainingConfig.ExecuteForColor == figureOn.Color)
            {
                _fileLogger.LogDebug($"Training: Find next book move to automatically respond");
                if (_openingTrainingConfig.Variations == OpeningBook.VariationsEnum.BestMove ||
                    bookMoveBases.Length == 1)
                {
                    _fileLogger?.LogDebug(
                        $"Training: set LEDs for execute best book move {bestBookMove.FromField}{bestBookMove.ToField} ");
                    _eChessBoard?.SetLEDsFor(new SetLEDsParameter()
                    {
                        FieldNames = new[] { bestBookMove.FromField, bestBookMove.ToField },
                        Promote = string.Empty,
                        IsMove = true,
                        IsEngineMove = true,
                        FenString = bestBookMove.FenPosition
                    });
                  
                    return;
                }
                
                var candidates = OpeningBook.GetCandidates(bookMoveBases, _openingTrainingConfig.Variations);
                var best = candidates.ToArray().OrderBy(m => (LineRandomness.Next(2) % 2) == 0)
                    .ToArray()[LineRandomness.Next(0, candidates.Length)];
                _fileLogger?.LogDebug($"Training: set LEDs for execute book move {best.FromField}{best.ToField} ");
                _eChessBoard?.SetLEDsFor(new SetLEDsParameter()
                {
                    FieldNames = new[] { best.FromField, best.ToField },
                    Promote = string.Empty,
                    IsMove = true,
                    IsEngineMove = true,
                    FenString = best.FenPosition
                });

                return;
            }

            foreach (var bookMoveBase in bookMoveBases)
            {
                _fileLogger?.LogDebug($"Training: enqueue possible book move {bookMoveBase.FromField}{bookMoveBase.ToField} ");
                _openingTrainingMoves.Enqueue(bookMoveBase);
            }
        }

        private void MenuItemShowGraphicScoreLine_OnClick(object sender, RoutedEventArgs e)
        {
            var dialog = new ConfigureGraphicalPositionsWindow(_configuration);
            dialog.Owner = this;
            dialog.ColorsChanged += Dialog_ColorsChanged;
        ...
        private void Dialog_ColorsChanged(object sender, EventArgs e)
        {
            Dispatcher?.Invoke(() =>
            {
                _engineWindow?.RefreshColors();
            });
            
        }

        private void MenuItemConnectChessnutMoveBoard_OnClick(object sender, RoutedEventArgs e)
        {
            ConnectDisconnectToBoard(ConnectToChessnutMove, DisconnectFromChessnutMove);
        }


        private void MenuItemConfigureChessnutMove_OnClick(object sender, RoutedEventArgs e)
        {
            if (_currentAction == CurrentAction.InRunningGame)
            {
                return;
            ...
            var winConfigureChessnut = new ConfigureChessnutMoveWindow(_configuration) { Owner = this };
            var showDialog = winConfigureChessnut.ShowDialog();
            if (showDialog.HasValue && showDialog.Value)
            {
                _lastEBoard = Constants.ChessnutMove;
        ...
        {
            if (_initialChessBackgroundFieldSize == 0 || !_sdiLayout)
            {
                return;
            }

            double maxFreeSpace = _configuration.GetBoolValue("resizeChessboard", true)
                ? this.ActualHeight - MenuMain.ActualHeight - dockPanelBelowBoard.ActualHeight
                : _initialFeeSpace;
            chessBoardUcGraphics.ChessBackgroundFieldSize = _initialChessBackgroundFieldSize + ((maxFreeSpace - _initialFeeSpace) / 8);
            chessBoardUcGraphics.ChessFieldSize = _initialChessFieldSize + ((maxFreeSpace - _initialFeeSpace) / 8);

        }

        private void Window_OnResizeChanged(object sender, SizeChangedEventArgs e)
        {
            ResizeChessboard();
        }

        private void BearChessMainWindow_OnStateChanged(object sender, EventArgs e)
        {
            ResizeChessboard();
        }

        private void MenuItemResizeChessboard_OnClick(object sender, RoutedEventArgs e)
        {
            var resizeChessboard = _configuration.GetBoolValue("resizeChessboard", false);
            resizeChessboard = !resizeChessboard;
            _configuration.SetBoolValue("resizeChessboard", resizeChessboard);
            imageResizeChessboard.Visibility = resizeChessboard ? Visibility.Visible : Visibility.Hidden;
            ResizeChessboard();
        }

        private void MenuItemShowSmallChessClock_OnClick(object sender, RoutedEventArgs e)
        {
            var showClock = _configuration.GetBoolValue("showSmallChessClock", false);
            showClock = !showClock;
            _configuration.SetBoolValue("showSmallChessClock", showClock);
            imageShowSmallChessClock.Visibility = showClock ? Visibility.Visible : Visibility.Hidden;
            if (_currentAction != CurrentAction.InRunningGame)
            {
                chessBoardUcGraphics.ShowSmallClocks(showClock);
            }
        }

        private void MenuItemChessCom_OnClick(object sender, RoutedEventArgs e)
        {
            var configureWindow = new ChessComConfigureWindow(_configuration) { Owner = this };
            var showDialog = configureWindow.ShowDialog();
            if (showDialog.HasValue && showDialog.Value)
            {
                _configuration.Save();
        ...
        private int SetLEDsDiff(string fen1, string fen2, int prevDiff)
        {
            if (string.IsNullOrWhiteSpace(fen2) || string.IsNullOrWhiteSpace(fen1))
            {
                return 0;
            }

            if (fen1.Equals(_prevFen1) && (fen2.Equals(_prevFen2)))
            {
                // _fileLogger?.LogDebug($"SetLEDsDiff: FEN are equal: {fen2}");
                return prevDiff;
            }
            ...
            var chessBoard = new ChessBoard();
            chessBoard.Init();
            chessBoard.SetPosition(fen1);
            var chessBoardBoard = new ChessBoard();
            chessBoardBoard.Init();
            chessBoardBoard.SetPosition(fen2);
            List<string> invalidFields = new List<string>();
            foreach (var boardField in Fields.BoardFields)
            {
                var bField1 = chessBoard.GetFigureOn(boardField);
                var bField2 = chessBoardBoard.GetFigureOn(boardField);
                if (bField1.FigureId != bField2.FigureId)
                {
                    invalidFields.Add(Fields.GetFieldName(boardField));
                }
            }


            var leds = new SetLEDsParameter()
            {
            ...
            _eChessBoard?.SetAllLEDsOff(true);
            _fileLogger?.LogDebug($"SetLEDsDiff: Send invalid fields {string.Join(" ",invalidFields)}");
            _eChessBoard?.SetLEDsFor(leds);
            return invalidFields.Count;
        }

        private void ShowNextPuzzle(PgnGame pgnGame, int playCount, string title, string url, DateTime dateIme, int totalCount=0, int solvedCount=0)
        {
            _waitForPuzzlePosition = true;
            _prevRequestForHelpFen = string.Empty;
            _fileLogger?.LogDebug($"Puzzle: Fen: {pgnGame.FENLine}{Environment.NewLine}Pgn: {pgnGame.GetGame()}{Environment.NewLine}");
            _chessBoard.NewGame();
            if (string.IsNullOrEmpty(pgnGame.GameEvent))
            {
                pgnGame.GameEvent = title;
            }
            _chessBoard.SetPosition(pgnGame.FENLine, true);
            _gameStartFenPosition = _chessBoard.GetFenPosition();
            chessBoardUcGraphics.UnMarkAllFields();
            chessBoardUcGraphics.ClearPosition();
            chessBoardUcGraphics.RepaintBoard(_chessBoard);
            _puzzlePgnGame = null;
            _puzzlePgnGameMoveIndex = 0;
            _puzzleHelpRequestedCount = 0;
            _puzzleMoves = new List<Move>();
            try
            {
                var pgnLoader = new PgnLoader();
                _puzzlePgnGame = pgnGame;
                chessBoardUcGraphics.SetPlayer(_puzzlePgnGame.PlayerWhite, _puzzlePgnGame.PlayerBlack, true);
                for (var i = 0; i < _puzzlePgnGame.MoveCount; i++)
                {
                    var move = _puzzlePgnGame.GetMove(i);
                    _chessBoard.MakePgnMove(move, string.Empty, string.Empty, string.Empty);
                }
                
                _puzzleMoves.AddRange(_chessBoard.GetPlayedMoveList());
                if (_puzzleMoves[0] == null)
                {
                    _puzzlePgnGameMoveIndex = 1;
                }
            }
            catch (Exception ex)
            {
            ...
            _chessBoard.NewGame();
            _chessBoard.SetPosition(pgnGame.FENLine, true);
            _puzzleWindow?.SetPuzzle(pgnGame, playCount > 0 ? playCount : pgnGame.MoveCount, dateIme, url, totalCount, solvedCount);
            _moveListWindow?.Clear();
            _moveListWindow?.SetStartPosition(pgnGame.FENLine);
            if (_puzzlePgnGame != null)
            {
                _moveListWindow?.SetPlayer(_puzzlePgnGame.PlayerWhite, Fields.COLOR_WHITE);
                _moveListWindow?.SetPlayer(_puzzlePgnGame.PlayerBlack, Fields.COLOR_BLACK);
            }
            ...
            if (_eChessBoard != null)
            {
                Dispatcher?.Invoke(() =>
                {
                    ...
                    var fenCode = _chessBoard.GetFenPosition();
                    _fileLogger?.LogDebug($"Waiting for position on board for {fenCode}");
                    if (_eChessBoard.ValidForAnalyse)
                    {
                      //  if (_eChessBoard.SelfMoving)
                        {
                        ...
                        progressWorker.DoWorkWithModal(progress =>
                        {
                            ...
                            int invalidFieldsCount = 0;
                            while (!splashProgressControlContents[0].Cancel &&
                                   !fenCode.StartsWith(eboardFen2) ||
                                   string.IsNullOrEmpty(eboardFen2))
                            {
                                // if (!_eChessBoard.SelfMoving)
                                
                                invalidFieldsCount = SetLEDsDiff(fenCode, eboardFen2, invalidFieldsCount);
                                _eChessBoard?.DisplayOnClock($" -{invalidFieldsCount} ");
                                eboardFen2 = _eChessBoard?.GetBoardFen();
                                Thread.Sleep(300);
                            }
                    ...
                    else
                    {
                        _eChessBoard?.SetFen(fenCode);
                        var eboardFen2 = _eChessBoard?.GetBoardFen();
                        SetLEDsDiff(fenCode, eboardFen2,0);
                        var messageBoxResult = BearChessMessageBox.Show(_rm.GetString("PlaceAlOnCorrectPosition"),
                            _rm.GetString("ConfirmPosition"), MessageBoxButton.OK, MessageBoxImage.Information);
                    }
                    _eChessBoard?.SetFen(fenCode);
                    _eChessBoard?.AllowTakeBack(true);
                    _eChessBoard?.SetDemoMode(true);
            ...
            _waitForPuzzlePosition = false;
        }

        
        private void NewPuzzleRequested(NewPuzzleRequest puzzleRequest) 
        {
            _currentNewPuzzleRequest = puzzleRequest;
            _currentPuzzleSource = puzzleRequest.PuzzleSource;
            _prevRequestForHelpFen = string.Empty;
            chessBoardUcGraphics.UnMarkAllFields(true);
            if (_currentPuzzleSource == PuzzleSource.Lichess)
            {
                _currentDatabasePuzzle = null;
                var lichessPuzzle = LichessPuzzleReader.GetPuzzle();
                ...
                chessBoard.Init();
                chessBoard.NewGame();
                chessBoard.SetPosition(lichessPuzzle.Fen);
                var pgnGame = new PgnGame();
                // First move is last move the to reach the puzzle position.
                // FEN need to changed after first move.
                string[] array = lichessPuzzle.Moves.Split(";".ToCharArray(),StringSplitOptions.RemoveEmptyEntries);
                for (int i = 0; i < array.Length; i++)
                {
                    string s = array[i];
                    chessBoard.MakeMove(s.Substring(0,2), s.Substring(2,2));
                    if (i==0)
                    {
                ...
                pgnGame.GameEvent = lichessPuzzle.Themes;
                ShowNextPuzzle(pgnGame, lichessPuzzle.PlayCount - 1, string.Empty, lichessPuzzle.GameUrl,
                    DateTime.MinValue, 0, 0);
                return;
            }
            if (_currentPuzzleSource == PuzzleSource.LichessOnline)
            {
                _currentDatabasePuzzle = null;
                var puzzleApi = new PuzzleApi(new LichessApiClient(_fileLogger), _fileLogger);
                var lichessPuzzle = _currentNewPuzzleRequest.Random ? AsyncHelper.RunSync(async() => await puzzleApi.GetRandomPuzzle())
                        : AsyncHelper.RunSync(async() => await puzzleApi.GetPuzzleOfTheDay());
                if (lichessPuzzle == null)
                {
                    return;
                }
                ...
                chessBoard.Init();
                chessBoard.NewGame();
                _fileLogger?.LogDebug($"lichess puzzle: Initial moves: {lichessPuzzle.Game.PGN}");
                string[] initMoves = lichessPuzzle.Game.PGN.Split(" ".ToCharArray(),StringSplitOptions.RemoveEmptyEntries);
                foreach (var initMove in initMoves)
                {
                    chessBoard.MakePgnMove(initMove,string.Empty,string.Empty,string.Empty);
                }
                var pgnGame = new PgnGame();
                pgnGame.FENLine = chessBoard.GetFenPosition();
                _fileLogger?.LogDebug($"lichess puzzle:  Fen after initial moves: {pgnGame.FENLine}");
                string[] array = lichessPuzzle.Solution.ToArray();
                chessBoard.Init();
                chessBoard.NewGame();
                chessBoard.SetPosition(pgnGame.FENLine);
                // var _ = chessBoard.GenerateMoveList();
                foreach (var s in array)
                {
                    chessBoard.MakeMove(s.Substring(0,2),s.Substring(2,2),s.Length > 4 ? s.Substring(4,1) : string.Empty);
                }
                ...
                pgnGame.GameEvent = string.Join(",",lichessPuzzle.Themes);
                ShowNextPuzzle(pgnGame, moveCnt, string.Empty, string.Empty, DateTime.MinValue,0,0);
                return;
            }
            if (_currentPuzzleSource == PuzzleSource.BearChess)
            {                
                {
                    var database = new PuzzleDatabase(_fileLogger, Path.Combine(_puzzleDBPath, PuzzleDatabase.BearChessDBFileName));
                    var total = database.GetTotalPuzzlesCount();
                    var solved = database.GetTotalPuzzlesCount(true);
                    if (total > 0 && total == solved)
                    {
                        database.ResetToUnsolved();
                        solved = 0;
                    }
                    int currentId = _currentDatabasePuzzle != null ? _currentDatabasePuzzle.Id : 0;
                    _currentDatabasePuzzle =
                        _currentNewPuzzleRequest.Random ? database.LoadRandomPuzzle() : database.LoadNextPuzzle(currentId);
                    if (_currentDatabasePuzzle != null && _currentDatabasePuzzle.Id>0)
                    {
                        ShowNextPuzzle(_currentDatabasePuzzle.PgnGame, _currentDatabasePuzzle.PlayCount, string.Empty, string.Empty, DateTime.MinValue,total,solved);
                        return;
                    }
                }
                return;
            ...
            if (_currentPuzzleSource == PuzzleSource.Database)
            {

                var puzzleDBFileName = _configuration.GetConfigValue("PuzzleDatabaseFileName", PuzzleDatabase.ImportDBFileName);
                var database = new PuzzleDatabase(_fileLogger, Path.Combine(_puzzleDBPath, puzzleDBFileName));
                var total = database.GetTotalPuzzlesCount();
                if (total == 0)
                {
                    _puzzleWindow.ShowMissingPuzzles();
                    return;
                }
                ...
                if (_currentDatabasePuzzle != null && _currentDatabasePuzzle.Id > 0)
                {
                    ShowNextPuzzle(_currentDatabasePuzzle.PgnGame, _currentDatabasePuzzle.PlayCount, string.Empty,
                        string.Empty, DateTime.MinValue, total, solved);
                    return;
                }
            ...
            if (_currentPuzzleSource == PuzzleSource.ChessCom)
            {
                _currentDatabasePuzzle = null;
                
                if (puzzleRequest.ByDate)
                {
                    if (puzzleRequest.SelectedDate.HasValue && !puzzleRequest.SelectedDate.Value.Date.Equals(DateTime.Now.Date))
                    {
                        _lastChessComDailyPuzzle = puzzleRequest.SelectedDate.Value;
                    }
                    if (_lastChessComDailyPuzzle.Date.Equals(DateTime.Now.Date))
                    {
                        _lastChessComDailyPuzzle = new DateTime(2007, 05, 06);
                    }
                    _lastChessComDailyPuzzle = _lastChessComDailyPuzzle.AddDays(1);
                    PuzzleOfDayRangeResponse[] response = null;
                    while (true)
                    {
                        response =  ChessComReader.GetPuzzleRange(_lastChessComDailyPuzzle, _lastChessComDailyPuzzle.AddDays(1));
                        if (response == null || response.Length == 0 || string.IsNullOrEmpty(response[0].Pgn))
                        {   
                            _fileLogger?.LogDebug(($"Puzzle ChessCom by date: No response for {_lastChessComDailyPuzzle}"));
                            _lastChessComDailyPuzzle = _lastChessComDailyPuzzle.AddDays(1);
                            if (_lastChessComDailyPuzzle.Date.Equals(DateTime.Now.Date))
                            {
                                return;
                    ...
                    _fileLogger?.LogDebug($"Puzzle: Title: {response[0].Title}{Environment.NewLine}Pgn: {response[0].Pgn}{Environment.NewLine}");
                    try
                    {
                        var pgnLoader = new PgnLoader();
                        ShowNextPuzzle(pgnLoader.GetGame(response[0].Pgn), 0, response[0].Title, string.Empty,
                ...
                var puzzleResponse = ChessComReader.GetPuzzle(_currentNewPuzzleRequest.Random);
                if (puzzleResponse == null) {

                    return;
                }
                _fileLogger?.LogDebug($"Puzzle: Title: {puzzleResponse.Title}{Environment.NewLine}Fen: {puzzleResponse.Fen}{Environment.NewLine}Pgn: {puzzleResponse.Pgn}{Environment.NewLine}Url: {puzzleResponse.Url}");
                try
                {
                    var pgnLoader = new PgnLoader();
                    ShowNextPuzzle(pgnLoader.GetGame(puzzleResponse.Pgn), 0, puzzleResponse.Title, puzzleResponse.Url, DateTimeOffset.FromUnixTimeSeconds(puzzleResponse.PublishTime).DateTime);
                    return;
                }
                catch (Exception ex)
                {
        ...
        private void HandlePuzzleFenPosition(string fenPosition)
        {
            if (_waitForPuzzlePosition || _puzzleMoves == null || _puzzlePgnGameMoveIndex > _puzzleMoves.Count - 1)
            {
                return;
            }
            ...
            if (_puzzleHelpRequestedCount > 0 && !_prevRequestForHelpFen.Equals(fenPosition))
            {
                _puzzleHelpRequestedCount = 0;
            }
            _prevRequestForHelpFen = fenPosition;
            if (_puzzleHelpRequestedCount > 0 )
            {
                var strings = fenPosition.Split(" ".ToCharArray());
                if (_chessBoard.GetFenPosition().StartsWith(strings[0]))
                {
                    PuzzleHintRequested(_currentPuzzleSource);
                    return;
                }
            }
            var puzzleMove = _puzzleMoves[_puzzlePgnGameMoveIndex];
            if (puzzleMove != null)
            {
                _fileLogger?.LogDebug(
                    $"Puzzle: current move {_puzzlePgnGameMoveIndex}: {puzzleMove.FromFieldName}{puzzleMove.ToFieldName}");
            }
            else
            {
                _fileLogger?.LogDebug($"Puzzle: No move for index {_puzzlePgnGameMoveIndex}");
            }

            bool match = puzzleMove != null && puzzleMove.Fen.StartsWith(fenPosition);
            if (match)
            {
                _puzzleHelpRequestedCount = 0;
                _fileLogger?.LogDebug($"Puzzle: Matching fen: {fenPosition}");
                _chessBoard.MakeMove(puzzleMove);
                var playedMoveList = _chessBoard.GetPlayedMoveList();
                Dispatcher?.Invoke(() =>
                {
                    _eChessBoard?.SetAllLEDsOff(true);
                    for (int i = _puzzlePgnGameMoveIndex; i < playedMoveList.Length; i++)
                    {
                        _moveListWindow?.AddMove(playedMoveList[i]);
                    }
                });
                _playedMoveList = playedMoveList;
                _puzzlePgnGameMoveIndex++;                
            }
            else
            {
                _fileLogger?.LogDebug($"Puzzle: Not matching fen: {fenPosition}");
                Dispatcher?.Invoke(() =>
                {
                    _eChessBoard?.SetAllLEDsOff(true);
                    if (_puzzleMoves.Any(p => p != null && !string.IsNullOrEmpty(p.Fen) && p.Fen.StartsWith(fenPosition)))
                    {
                        _fileLogger?.LogDebug($"Puzzle: But found another matching move!?");
                        return;
                    }
                    var move = _chessBoard.GetMove(fenPosition, true);
                    _fileLogger?.LogDebug($"Puzzle: Generated move {move}");
                    if (move.Length >= 4)
                    {
                        var fromField = move.Substring(0, 2);
                        var toField = move.Substring(2, 2);
                        _puzzleWindow.IncTries();
                        var ledsParam = new SetLEDsParameter()
                        {
                            FenString = _chessBoard.GetFenPosition(),
                            IsError = true,
                            IsMove = false,
                            IsEngineMove = true,
                            InvalidFieldNames = new string[] { fromField, toField }
                        };
                        _eChessBoard?.SetLEDsFor(ledsParam);
                        _eChessBoard?.BuzzerOnInvalid();
                        _eChessBoard?.DisplayOnClock($"{_chessBoard.GetFigureOn(Fields.GetFieldNumber(fromField)).FenFigureCharacter} {toField}-{fromField}");
                        if (_eChessBoard != null && _eChessBoard.SelfMoving)
                        {
                            _fileLogger?.LogDebug($"Puzzle: Reset board to current board position");
                            _eChessBoard?.SetFen(_chessBoard.GetFenPosition());
                        }
                    }
                });
            ...
            Dispatcher?.Invoke(() =>
            {
                ...
                if (_puzzlePgnGameMoveIndex > _puzzleMoves.Count - 1)
                {
                    UpdatePuzzleDatabase(_currentPuzzleSource, _currentDatabasePuzzle);
                    if (_currentNewPuzzleRequest.PuzzleSource == PuzzleSource.ChessCom &&
                        _currentNewPuzzleRequest.ByDate)
                    {
                        _configuration.SetDateValue("lastChessComDailyPuzzle", _lastChessComDailyPuzzle);
                    }
                    _puzzleWindow.PuzzleSolved();
                }
            });
            if (_puzzlePgnGameMoveIndex > _puzzleMoves.Count - 1)
            {
                ...
                _eChessBoard?.SetAllLEDsOff(true);
                _eChessBoard?.DisplayOnClock(_rm.GetString("Solved"));
            }
        }

        private void MenuItemStartPuzzle_OnClick(object sender, RoutedEventArgs e)
        {
            ...
            if (_eChessBoard != null && !_eChessBoard.ValidForAnalyse)
            {
                BearChessMessageBox.Show(_rm.GetString("BoardNotSuitableForPuzzle"), _rm.GetString("NotSuitable"),
                    MessageBoxButton.OK,
                    MessageBoxImage.Error);
                return;
            }
            PuzzleDatabase.InstallBearChessDatabase(_binPath, _dbPath, _fileLogger);
            _currentDatabasePuzzle = null;
            menuItemSetupPosition.IsEnabled = false;
            menuItemAnalyseMode.IsEnabled = false;
            menuItemEngineMatch.IsEnabled = false;
            ...
            menuItemStartPuzzle.IsEnabled = false;
            _currentAction = CurrentAction.InPuzzleMode;
            _currentPuzzleSource = (PuzzleSource)_configuration.GetIntValue("puzzleSource", (int)PuzzleSource.ChessCom);

            textBlockRunningMode.Text = _rm.GetString("ModePuzzle");
            if (_puzzleWindow == null)
            {
                _puzzleWindow = new PuzzleWindow(_configuration, _puzzleDBPath, _fileLogger);
                _puzzleWindow.NextPuzzle += PuzzleWindow_NextPuzzle;
                _puzzleWindow.ResetDatabase += PuzzleWindow_ResetDatabase;
                _puzzleWindow.SolutionRequested += PuzzleWindow_SolutionRequested;
                _puzzleWindow.HintRequested += PuzzleWindow_HintRequested;
                _puzzleWindow.Closed += PuzzleWindow_Closed;
                _puzzleWindow.Show();
            }
            ...
            _eChessBoard?.DisplayOnClock($"Puzzle");
        }

        private void PuzzleWindow_ResetDatabase(object sender, PuzzleSource puzzleSource)
        {
            if (puzzleSource != PuzzleSource.BearChess && puzzleSource != PuzzleSource.Database)
            {
                return;
            }
            var puzzleDBFileName = _configuration.GetConfigValue("PuzzleDatabaseFileName", PuzzleDatabase.ImportDBFileName);
            var dbName = puzzleSource == PuzzleSource.BearChess ? PuzzleDatabase.BearChessDBFileName : puzzleDBFileName;
            var database = new PuzzleDatabase(_fileLogger, Path.Combine(_puzzleDBPath, dbName));
            try
            {
                database.ResetToUnsolved();
                if (_currentDatabasePuzzle != null)
                {
                    _currentDatabasePuzzle.Id = 0;
                }

                Dispatcher?.Invoke(() =>
                {
                    _puzzleWindow.ResetSolvedCount();
                });
        ...
        private void PuzzleModeEnd()
        {
            try
            {
                _configuration.SetIntValue("puzzleSource", (int)_currentPuzzleSource);
                _lastChessComDailyPuzzle = _configuration.GetDateValue("lastChessComDailyPuzzle", new DateTime(2007, 05, 07));
                if (_puzzleWindow != null)
                {
                    _puzzleWindow.NextPuzzle -= PuzzleWindow_NextPuzzle;
        ...
        private void PuzzleWindow_Closed(object sender, EventArgs e)
        {
            PuzzleModeEnd();
        }

        private void UpdatePuzzleDatabase(PuzzleSource puzzleSource, DatabasePuzzle databasePuzzle)
        {
            if (puzzleSource != PuzzleSource.BearChess && puzzleSource != PuzzleSource.Database)
            {
                return;
            }
            var fileName = _configuration.GetConfigValue("PuzzleDatabaseFileName", PuzzleDatabase.ImportDBFileName);
            var dbName = puzzleSource == PuzzleSource.BearChess ?  PuzzleDatabase.BearChessDBFileName : fileName;
            var database = new PuzzleDatabase(_fileLogger, Path.Combine(_puzzleDBPath, dbName));
            if (databasePuzzle != null)
            {
                database.UpdateSolved(databasePuzzle.Id, true);
            }
        }

        private void MakeAnswerMove()
        {            
            if (_puzzlePgnGameMoveIndex < _puzzleMoves.Count)
            {
                var fromField = string.Empty;
                var toField = string.Empty;
                var puzzleMove = _puzzleMoves[_puzzlePgnGameMoveIndex];
                _fileLogger?.LogDebug($"Puzzle: Answer move {_puzzlePgnGameMoveIndex}: {puzzleMove.FromFieldName}{puzzleMove.ToFieldName}");
                var figureCharacter = _chessBoard.GetFigureOn(puzzleMove.FromField).FigureCharacter;
                _chessBoard.MakeMove(puzzleMove);
               
                var playedMoveList = _chessBoard.GetPlayedMoveList();
                Dispatcher?.Invoke(() =>
                {
                    for (var i = _puzzlePgnGameMoveIndex; i < playedMoveList.Length; i++)
                    {
                        _moveListWindow?.AddMove(playedMoveList[i]);
                        fromField = playedMoveList[i].FromFieldName;
                        toField = playedMoveList[i].ToFieldName;
                    }
                });
                _playedMoveList = playedMoveList;
                _puzzlePgnGameMoveIndex++;
                Dispatcher?.Invoke(() =>
                {
                    chessBoardUcGraphics.UnMarkAllFields(true);
                    chessBoardUcGraphics.RepaintBoard(_chessBoard);
                    chessBoardUcGraphics.MarkLastMoveFields(new[] { puzzleMove.FromField, puzzleMove.ToField });
                    _eChessBoard?.SetLEDsFor(new SetLEDsParameter()
                    {
                        FenString = _chessBoard.GetFenPosition(),
                        IsError = false,
                        IsMove = true,
                        IsEngineMove = true,
                        FieldNames = new string[] { fromField, toField }
                    });
                    string capSign = puzzleMove.CapturedFigure == FigureId.NO_PIECE ? "-" : "x";
                    _eChessBoard?.DisplayOnClock($"{puzzleMove.GetMoveString(_eBoardLongMoveFormat, _displayCountryType)}");
                });                
            }          
        }

        private void PuzzleHintRequested(PuzzleSource puzzleSource)
        {
            _currentPuzzleSource = puzzleSource;
            if (_puzzlePgnGameMoveIndex < _puzzleMoves.Count)
            {
                var fromField = string.Empty;
                var toField = string.Empty;
                var puzzleMove = _puzzleMoves[_puzzlePgnGameMoveIndex];
                _fileLogger?.LogDebug($"Puzzle: Hint requested on index {_puzzlePgnGameMoveIndex}: {puzzleMove.FromFieldName}{puzzleMove.ToFieldName}");
                if (_puzzleHelpRequestedCount < 2)
                {
                    Dispatcher?.Invoke(() =>
                    {
                        chessBoardUcGraphics.MarkField(puzzleMove.FromField);
                        _eChessBoard?.SetLEDsFor(new SetLEDsParameter()
                        {
                            IsError = false,
                            IsMove = false,
                            IsEngineMove = false,
                            IsThinking = true,
                            FieldNames = new string[] { puzzleMove.FromFieldName },
                            HintFieldNames = new string[] { puzzleMove.FromFieldName }
                        });
                        _eChessBoard?.DisplayOnClock($" {puzzleMove.FromFieldName}");
                    });
                    return;
                }
                else
                {
                    Dispatcher?.Invoke(() =>
                    {
                        ...
                        _eChessBoard?.SetLEDsFor(new SetLEDsParameter()
                        {
                            ...
                            IsEngineMove = false,
                            IsThinking = true,
                            FieldNames = new string[] { puzzleMove.FromFieldName, puzzleMove.ToFieldName },
                            HintFieldNames = new string[] { puzzleMove.FromFieldName, puzzleMove.ToFieldName }
                        });                    
                        string capSign = puzzleMove.CapturedFigure == FigureId.NO_PIECE ? "-" : "x";
                        _eChessBoard?.DisplayOnClock($"{puzzleMove.GetMoveString()}");
                    });
                    _puzzleHelpRequestedCount = 0;
                    return;
                }           
            ...
            if (_puzzlePgnGameMoveIndex >= _puzzleMoves.Count)
            {
                UpdatePuzzleDatabase(puzzleSource, _currentDatabasePuzzle);
                _currentDatabasePuzzle = null;
            }
        }

        private void PuzzleWindow_HintRequested(object sender, PuzzleSource puzzleSource)
        {

            _puzzleHelpRequestedCount++;
            PuzzleHintRequested(puzzleSource);
        }

        private void PuzzleWindow_NextPuzzle(object sender, NewPuzzleRequest puzzleRequest)
        {
            NewPuzzleRequested(puzzleRequest);
        }
        

        private void PuzzleWindow_SolutionRequested(object sender, PuzzleSource puzzleSource)
        {
            _puzzleHelpRequestedCount = 0;
            _currentPuzzleSource = puzzleSource;
            for (var i = _puzzlePgnGameMoveIndex; i < _puzzleMoves.Count; i++)
            {
                _chessBoard.MakeMove(_puzzleMoves[i]);
            ...
            _playedMoveList = playedMoveList;
            _puzzlePgnGameMoveIndex = _puzzleMoves.Count - 1;
            Dispatcher?.Invoke(() =>
            {
                chessBoardUcGraphics.UnMarkAllFields(true);
                chessBoardUcGraphics.RepaintBoard(_chessBoard);
        ...
        private void MenuItemImportPuzzle_OnClick(object sender, RoutedEventArgs e)
        {
            ...
            if (showDialog.Value && !string.IsNullOrWhiteSpace(openFileDialog.FileName))
            {
                try
                {
                    var matePattern = PuzzleImporter.LoadFromFile(openFileDialog.FileName);
                    var database2 = new PuzzleDatabase(_fileLogger, Path.Combine(_puzzleDBPath, PuzzleDatabase.BearChessDBFileName));
                    foreach (var matePatternProblem in matePattern.Problems)
                    {

                        var chessBoard = new ChessBoard();
                        chessBoard.Init();
                        chessBoard.NewGame();
                        chessBoard.SetPosition(matePatternProblem.Fen);
                        chessBoard.GenerateMoveList();
                        var pgnGame = new PgnGame();
                        if (!string.IsNullOrWhiteSpace(matePatternProblem.Fen))
                        {
                            pgnGame.AddValue("FEN", matePatternProblem.Fen);
                            pgnGame.AddValue("SetUp", "1");
                        }
                        foreach (var s in matePatternProblem.Moves.Split(";".ToCharArray()))
                        {
                        ...
                        pgnGame.GameEvent = matePatternProblem.MateType;
                        database2.SavePuzzle("", pgnGame.GameEvent, pgnGame.GetGame(), moveCnt);
                    }                                      
                }
                catch (Exception ex)
                {
                    _fileLogger.LogError(ex);
                    var messageBoxResult = BearChessMessageBox.Show(ex.Message, 
                        _rm.GetString("Error"),
                        MessageBoxButton.OK, MessageBoxImage.Error);
                }                
            }
        ...
        {
            var configureWindow = new LichessConfigureWindow(_fileLogger) { Owner = this };
            var showDialog = configureWindow.ShowDialog();
            if (showDialog.HasValue && showDialog.Value)
            {
                _configuration.Save();
        ...
        private async void MenuItemGetCurrentGames_OnClick(object sender, RoutedEventArgs e)
        {
            var gamesApi = new GamesApi(new LichessApiClient(_fileLogger), _fileLogger);
            try
            {
                var games = await gamesApi.GetOngoingGames();
                if (games.OngoingGames.Length > 0)
                {
                    SetFenPosition(games.OngoingGames[0].Fen);
                }
            }
            catch
            {
        ...
        private void MenuItemConfigureArrows_OnClick(object sender, RoutedEventArgs e)
        {
            var configureWindow = new ConfigureArrowColorsWindow(_configuration) { Owner = this };
            configureWindow.ValueChangeEvent += ConfigureWindowOnValueChangeEvent;
            configureWindow.Show();
        }

        private void ConfigureWindowOnValueChangeEvent(object sender, EventArgs e)
        {
            chessBoardUcGraphics.ShowPossibleMoves(_configuration.GetBoolValue("showPossibleMoves", false));
            _showBestMove = _configuration.GetBoolValue("ShowBestMove", false);
            _showBestMove2 = _configuration.GetBoolValue("ShowBestMove2", false);
            _showBestMove3 = _configuration.GetBoolValue("ShowBestMove3", false);
            _showBestMoveN = _configuration.GetBoolValue("ShowBestMoveN", false);
            _showBestBookMove = _configuration.GetBoolValue("ShowBestBookMove", false);
            _showLastMove  = _configuration.GetBoolValue("ShowLastMove", false);
            _bookMoveArrowColor = _configuration.GetColorValue("BookArrowColor", Brushes.CadetBlue);
            _arrowColor = _configuration.GetColorValue("ArrowColor", Brushes.Khaki);
        }
        
        public SolidColorBrush GetColor(int engineIndex, SolidColorBrush defaultColor)
        {
            string key =  "BestMove1ArrowColor";
            switch ( engineIndex)
            {
                case 0: key = "BestMove1ArrowColor";
                    break;
                case 1: key = "BestMove2ArrowColor";
                    break;
                case 2: key = "BestMove3ArrowColor";
                    break;
                default: key = "BestMoveNArrowColor";
                    break;
            }
            return _configuration.GetColorValue(key,defaultColor);            
        }

        private void MenuItemTrainRepertoire_OnClick(object sender, RoutedEventArgs e)
        {
            if (_inRepertoireTrainingMode)
            {
                _trainingFenCodes.Clear();
                ...
                menuItemStartPuzzle.IsEnabled = true;
                menuItemTrainStartOpening.IsEnabled = true;
                menuItemTrainStopOpening.IsEnabled = true;
                menuItemTrainStartRepertoire.Visibility = Visibility.Visible;
                menuItemTrainStopRepertoire.Visibility = Visibility.Collapsed;
                textBlockRunningMode.Text = _rm.GetString("ModeEasyPlaying");
                _currentDatabaseRepertoire = null;
                while (_openingTrainingMoves.TryDequeue(out _)) { ; }
                try
                {
                    _trainingThread.Abort();
            ...
            if (_eChessBoard == null)
            {
                BearChessMessageBox.Show(_rm.GetString("ConnectFirst"), _rm.GetString("ElectronicBoards"),
                    MessageBoxButton.OK,
                    MessageBoxImage.Stop);
                return;
            }
            ...
            _repertoireTrainingConfig = _configuration.LoadRepertoireTrainingConfig();
            _currentDatabaseRepertoire = null;
            var dialogWindow =
                new RepertoireTrainingStartWindow(_repertoireDBPath, _repertoireTrainingConfig, _fileLogger)
                {
                    Owner = this
                };
            var showDialog = dialogWindow.ShowDialog();
            if (showDialog.HasValue && showDialog.Value)
            {
                _configuration.SaveRepertoireTrainingConfig(_repertoireTrainingConfig);
                _currentDatabaseRepertoire = new RepertoireDatabase(_fileLogger,
                    Path.Combine(_repertoireDBPath, _repertoireTrainingConfig.DatabaseName),this);
                _currentDatabaseRepertoire.LoadDb();
                _eChessBoard?.SetAllLEDsOff(true);
                _chessBoard.NewGame();
                _chessBoard.GenerateMoveList();
                chessBoardUcGraphics.RepaintBoard(_chessBoard);
                _currentAction = CurrentAction.InTrainingSetupMode;
                var eBoardFen = _eChessBoard?.GetBoardFen();
                if (!FenCodes.BasePosition.StartsWith(eBoardFen) || string.IsNullOrEmpty(eBoardFen))
                {
                    _eChessBoard?.NewGame();
                    if (_eChessBoard.ValidForAnalyse)
                    {
                        ...
                        progressWorker.DoWorkWithModal(progress =>
                        {
                            var eboardFen2 = _eChessBoard?.GetBoardFen();
                            while (!splashProgressControlContents[0].Cancel &&
                                   !FenCodes.BasePosition.StartsWith(eboardFen2) ||
                                   string.IsNullOrEmpty(eboardFen2))
                            {
                                eboardFen2 = _eChessBoard?.GetBoardFen();
                                Thread.Sleep(300);
                    ...
                    {
                        BearChessMessageBox.Show(_rm.GetString("PlaceAlOnCorrectPosition"),
                            _rm.GetString("ConfirmPosition"), MessageBoxButton.OK,
                            MessageBoxImage.Information);
                    }
                }
                ...
                _currentGame = null;
                menuItemTrainStartOpening.IsEnabled = false;
                menuItemTrainStopOpening.IsEnabled = false;
                menuItemTrainStartRepertoire.Visibility = Visibility.Collapsed;
                menuItemTrainStopRepertoire.Visibility = Visibility.Visible;
                _currentAction = CurrentAction.InEasyPlayingMode;
                _currentWhitePlayer = string.Empty;
                _currentBlackPlayer = string.Empty;
                _timeControlWhite = TimeControlHelper.GetTrainingTimeControl();
                ...
                _gameStartFenPosition = string.Empty;
                _inTrainingMode = true;
                _inRepertoireTrainingMode = true;
                _pureEngineMatch = false;
                _pureEngineMatchStoppedByBearChess = false;
                _gameAgainstEngine = false;
                _playersColor[Fields.COLOR_WHITE] = true;
                ...
                menuItemEloEstimateEngineMatch.IsEnabled = false;
                _prevTrainingFenPosition = string.Empty;
                _trainingThread = new Thread(HandleRepertoireOpeningTraining) { IsBackground = true };
                _trainingThread.Start();
                HandleRepertoireOpeningTraining(_chessBoard.GetFenPosition(), _prevFenPosition);
                if (_repertoireTrainingConfig.ShowCurrentGame && _moveListWindow == null)
                {
                    if (_sdiLayout)
                    {
