...
namespace www.SoLaNoSoft.com.BearChessWin
{
    ...
    public partial class ChessClockSimpleWindow : Window , IChessClocksWindow
    {
        ...
        public ChessClockSimpleWindow(string capture, Configuration configuration, double top, double left, ILogging logging)
        {
            InitializeComponent();
            _fileLogger = logging;
        ...
        public void SetTime(int hh, int mm, int ss, int extraSeconds = 0)
        {
            ...
            Title = _extraSeconds > 0 ? $"{_rm.GetString("Clock")} {_capture} ({hh:00}:{mm:00}:{ss:00} + {extraSeconds}s)" : $"{_rm.GetString("Clock")} {_capture} ({hh:00}:{mm:00}:{ss:00})";
            ToolTip = Title;
            borderWarning.Visibility = Visibility.Hidden;
            _duration = TimeSpan.Zero;
        }
