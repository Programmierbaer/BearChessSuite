...
namespace www.SoLaNoSoft.com.BearChess.EChessBoard
{
    public abstract class AbstractEBoardWrapper : IEBoardWrapper
    {
        ...
        protected AbstractEBoardWrapper(string name, string basePath)
        {
            ...
            _board.GameEndEvent += _board_GameEndEvent;
        }

        private void _board_GameEndEvent(object sender, string e)
        {
            GameEndEvent?.Invoke(sender, e);
        }
        ...
        private void HandleBoard()
        {
            ...
            while (!_stopCommunication)
            {
                ...
                if (piecesFen.WhiteWinsByKingPosition || piecesFen.BlackWinsByKingPosition ||
                    piecesFen.DrawByKingPosition)
                {
                   
                    GameEndEvent?.Invoke(this,piecesFen.WhiteWinsByKingPosition? "1-0" : piecesFen.BlackWinsByKingPosition ? "0-1" : "1/2");
                }
        ...
        private DataFromBoard GetPiecesFenAndSetLEDsForInvalidField(bool waitMove)
        {
            try
            {
                ...
                if (!fenPosition.IsFieldDump || (_board != null && _board.UseFieldDumpForFEN))
                {
                    ...
                    if (_board != null && _board.PieceRecognition)
                    {
                        fenPosition.WhiteWinsByKingPosition = internalChessBoard.WhiteWinsByKingPosition;
                        fenPosition.BlackWinsByKingPosition = internalChessBoard.BlackWinsByKingPosition;
                        fenPosition.DrawByKingPosition = internalChessBoard.DrawByKingPosition;
                        if (fenPosition.BlackWinsByKingPosition || fenPosition.WhiteWinsByKingPosition ||
                            fenPosition.DrawByKingPosition)
                        {
